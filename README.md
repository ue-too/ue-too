...
The [design document](https://hackmd.io/@niuee/ByKskjAUp) contains a more detailed explanation of the underlying mechanisms of the canvas including the camera system that is essentially the backbone of this custom canvas. Board used to be called vCanvas and vCamera is the same as the `BoardCamera`. There was a name change for the package from vCanvas to board.
...
For example using the iife bundle
```html
...
libraries rely on the `requestAnimationFrame` function to make it move so does the @niuee/board. 
By default the `Board` class does not call the `requestAnimationFrame` function by itself; it relies on 
