 ...
 */
export type HierarchicalStatePath<
    ParentStates extends string,
    ChildStates extends string,
> = ParentStates | `${ParentStates}.${ChildStates}`;
...
> extends TemplateState<
    ...
    EventOutputMapping
> {
    protected _childStateMachineConfig: ChildStateMachineConfig<
        EventPayloadMapping,
        Context,
        ChildStates,
        EventOutputMapping
     ...
     */
    protected abstract getChildStateMachine(): ChildStateMachineConfig<
        EventPayloadMapping,
        Context,
        ChildStates,
        EventOutputMapping
    ...
    getCurrentChildState(): ChildStates | null {
        ...
        const stateMachine = this._childStateMachineConfig
            .stateMachine as TemplateStateMachine<
            EventPayloadMapping,
            Context,
            ChildStates,
            EventOutputMapping
        ...
        return current === 'INITIAL' || current === 'TERMINAL'
            ? null
            : (current as ChildStates);
    }
    ...
    override uponEnter(
        ...
        const childStateToEnter =
            config.rememberHistory && this._historyState
                ? this._historyState
                : config.defaultChildState;

        // Start child state machine and transition to the appropriate child state
        const childMachine = config.stateMachine as TemplateStateMachine<
            EventPayloadMapping,
            Context,
            ChildStates,
            EventOutputMapping
    ...
    override beforeExit(
        ...
        if (this._childStateMachineConfig) {
            ...
            if (currentChild !== null) {
                const childState =
                    this._childStateMachineConfig.stateMachine.states[
                        currentChild
                    ];
                childState.beforeExit(
                    context,
                    this._childStateMachineConfig.stateMachine,
                    'TERMINAL'
                );
            }
