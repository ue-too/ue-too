...
export type TransitionDefinitionUnion<
    ...
    > = DefaultOutputMapping<EventPayloadMapping>,
> = {
    [K in keyof EventPayloadMapping]: TransitionDefinition<
        Context,
        EventPayloadMapping,
        K,
        StateNames,
...
> {
    ...
    transitions: TransitionDefinitionUnion<
        Context,
        EventPayloadMapping,
        StateNames,
        EventOutputMapping
...
>(
    ...
    EventOutputMapping
> {
    return schema as unknown as StateMachineSchema<
        Context,
        EventPayloadMapping,
        ExtractStateNames<States>,
        EventOutputMapping
...
>(
    schema: Schema,
    context: Schema extends StateMachineSchema<infer C, any, any, any>
        ? C
        : BaseContext
    ...
    type SchemaEventOutputMapping =
        Schema extends StateMachineSchema<any, any, any, infer EOM> ? EOM : any;
            ...
            if (transition.guards) {
                for (const guard of transition.guards) {
                    ...
                    if (typeof guard.guard === 'string') {
                        if (
                            !stateDef.guards ||
                            !(guard.guard in stateDef.guards)
                        ) {
                            throw new Error(
    ...
    const stateInstances: Record<
        string,
        TemplateState<any, SchemaContext, any>
    > = {};
    ...
    for (const stateDef of schema.stateDefinitions) {
        ...
        if (stateDef.guards) {
            for (const [guardName, guardFunction] of Object.entries(
                stateDef.guards
            )) {
    ...
    return new TemplateStateMachine<
        SchemaEventPayloadMapping,
        SchemaContext,
        any,
        SchemaEventOutputMapping
    >(stateInstances as any, schema.initialState as any, context, true) as any;
}
