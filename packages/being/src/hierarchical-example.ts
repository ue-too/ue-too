 ...
 */
import { CompositeState, HierarchicalStateMachine } from './hierarchical';
import {
    BaseContext,
    EventReactions,
    TemplateState,
...
class BufferingState extends TemplateState<
    MediaPlayerEvents,
    MediaPlayerContext,
    PlayingChildStates
> {
    ...
    uponEnter(
        context: MediaPlayerContext,
        stateMachine: any,
        from: PlayingChildStates | 'INITIAL'
    ) {
...
class StreamingState extends TemplateState<
    MediaPlayerEvents,
    MediaPlayerContext,
    PlayingChildStates
> {
    ...
    beforeExit(
        context: MediaPlayerContext,
        stateMachine: any,
        to: PlayingChildStates | 'TERMINAL'
    ) {
...
> {
    ...
    protected _eventReactions = {
        ...
        MediaPlayerContext,
        TopLevelStates
    > as EventReactions<MediaPlayerEvents, MediaPlayerContext, TopLevelStates>;
    ...
    protected getChildStateMachine() {
        ...
        if (!this._context) {
            throw new Error(
                'Context must be set on CompositeState before getChildStateMachine is called'
            );
        }
...
class PausedState extends TemplateState<
    MediaPlayerEvents,
    MediaPlayerContext,
    TopLevelStates
> {
...
export function runHierarchicalStateMachineExample() {
    ...
    machine.onStateChange((currentState, nextState) => {
        ...
        console.log(
            `[Active Path] [${machine.getActiveStatePath().join(' → ')}]\n`
        );
    });
    ...
    console.log(
        `   Is in PLAYING.BUFFERING? ${machine.isInStatePath('PLAYING.BUFFERING')}\n`
    );
    ...
    console.log(
        '5. Resuming (transitions back to PLAYING, remembers STREAMING)'
    );
    machine.happens('play', { track: 'song.mp3' });
    console.log(`   Current path: ${machine.getCurrentStatePath()}\n`);
    ...
    console.log(
        '8. Stopping (handled by parent PLAYING state, transitions to IDLE)'
    );
