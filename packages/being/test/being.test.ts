import { TemplateStateMachine, TemplateState, BaseContext, EventReactions, createStateGuard, Guard, EventGuards } from "../src/interface";
...
describe("being", ()=>{
    ...
    describe("Initialization", () => {
        it("should call uponEnter for initial state when state machine is constructed", () => {
            ...
            expect(idleState.uponEnter).toHaveBeenCalledWith(testContext, stateMachine, "IDLE");
            expect(stateMachine.currentState).toBe("IDLE");
        });
    ...
    describe("Lifecycle Methods", () => {
        it("should call wrapup() and transition to TERMINAL state", () => {
            ...
            stateMachine.wrapup();
            expect(idleState.beforeExit).toHaveBeenCalledWith(testContext, stateMachine, "TERMINAL");
        ...
        it("should not call beforeExit if already in TERMINAL state", () => {
            ...
            expect(idleState.beforeExit).toHaveBeenCalledTimes(beforeExitCallCount);
        });
    ...
    describe("onHappens Callbacks", () => {
        it("should call onHappens callback when event occurs", () => {
            ...
            expect(happensCallback).toHaveBeenCalledWith(["EVENT_1"], testContext);
        });
        ...
        it("should call onHappens callback with event payload", () => {
            ...
            expect(happensCallback).toHaveBeenCalledWith(["EVENT_WITH_DATA", { data: "test" }], testContext);
        });
    ...
    describe("Guards and Event Guards", () => {
        ...
        class GuardIdleState extends TemplateState<GuardEvents, GuardContext, GuardStates> {
            ...
            protected _eventGuards: Partial<EventGuards<GuardEvents, GuardStates, GuardContext, typeof this._guards>> = {
                toggle: [
        ...
        it("should fall back to defaultTargetState if no guard matches", () => {
            ...
            class GuardIdleStateWithNoMatch extends TemplateState<GuardEvents, GuardContext, GuardStates> {
                ...
                protected _eventGuards: Partial<EventGuards<GuardEvents, GuardStates, GuardContext, typeof this._guards>> = {
                    toggle: [
                        { guard: "alwaysFalse", target: "PAUSED" }
                    ]
                };
            }
    ...
    describe("Edge Cases", () => {
        it("should not transition if nextState is the same as current state", () => {
            ...
            expect(idleState.beforeExit).toHaveBeenCalledTimes(beforeExitCallCount);
            expect(idleState.uponEnter).toHaveBeenCalledTimes(uponEnterCallCount);
        });
