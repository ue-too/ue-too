...
export class OrthoGridSystem implements System {
    ...
    constructor(coordinator: Coordinator) {
        ...
        const gridComponentType = this.coordinator.getComponentType(ORTHO_GRID_COMPONENT); 
        if(gridComponentType === null) {
            throw new Error('OrthoGridComponent not registered');
        }
        const gridLocationComponentType = this.coordinator.getComponentType(ORTHO_GRID_LOCATION_COMPONENT);
        ...
        this.coordinator.setSystemSignature(ORTHO_GRID_SYSTEM, (1 << gridLocationComponentType) | (1 << zoneComponentType));
    }

    addEntityToGridCell(grid: Entity, row: number, column: number, entity: Entity, direction: 'top' | 'bottom' = 'top', displace: boolean = false): void {
        const gridComponent = this.coordinator.getComponentFromEntity<OrthoGridComponent>(ORTHO_GRID_COMPONENT, grid);
        if(!gridComponent) {
            return;
        }
        if(row < 0 || row >= gridComponent.rows || column < 0 || column >= gridComponent.columns) {
            return;
        }
        let zoneAtCell: Entity | null = null;
        for(const entity of this.entities) {
            const gridLocationComponent = this.coordinator.getComponentFromEntity<OrthoGridLocationComponent>(ORTHO_GRID_LOCATION_COMPONENT, entity);
            if(!gridLocationComponent) {
                continue;
            }
            if(gridLocationComponent.grid === grid && gridLocationComponent.row === row && gridLocationComponent.column === column) {
                zoneAtCell = entity;
                break;
            }
        }
    ...
    getEntireGridEntities(grid: Entity): {entities: (Entity | null)[][], hasHole: boolean} | null{
        ...
        const entities: (Entity | null)[][] = Array.from({ length: gridComponent.rows }, () => Array.from({ length: gridComponent.columns }, () => null));
        ...
        for(const entity of this.entities) {
            ...
            entities[gridLocationComponent.row][gridLocationComponent.column] = entity;
        }
        let hasHole = false;
