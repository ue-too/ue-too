 ...
 * ```
 */
export class NumberModificationEffect<
...
> implements Effect {
    ...
    constructor(
        ...
        if (typeof amountOrComponentName === 'number') {
            ...
            this._operation =
                (operationOrValuePath as 'add' | 'subtract' | 'set') || 'add';
            this._isTyped = false;
        } else {
    ...
    apply(): void {
        ...
        if (hasCustomSchema) {
            ...
            if (typeof component[field.name] === 'number') {
                if (this._operation === 'add') {
                    component[field.name] =
                        (component[field.name] as number) + this._amount;
            ...
            const mutableComponent = component as unknown as Record<
                string,
                unknown
            >;
...
export class TypeModificationEffect<T extends string> implements Effect {
    ...
    apply(): void {
        ...
        if (
            this._allowedValues &&
            !this._allowedValues.includes(this._newType)
        ) {
            return;
        }
...
export class EntityFieldModificationEffect implements Effect {
    ...
    apply(): void {
        ...
        if (typeof component[field.name] === 'number') {
            component[field.name] =
                (component[field.name] as number) + this._newEntityValue;
        } else {
            return;
        }
    }
