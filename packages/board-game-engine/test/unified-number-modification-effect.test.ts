...
describe('NumberModificationEffect - Unified (Schema and Typed)', () => {
    ...
    describe('with custom schema components', () => {
        it('should work with schema-based components using old API', () => {
            ...
            const effect = new NumberModificationEffect(
                coordinator, 25, HEALTH_COMPONENT_SCHEMA, entity, 'health', 'subtract'
            );
            effect.apply();

            const updatedComponent = coordinator.getComponentFromEntity<Record<string, unknown>>(HEALTH_COMPONENT_SCHEMA, entity);
        ...
        it('should detect schema automatically and use schema-based approach', () => {
            ...
            const effect = new NumberModificationEffect<Record<string, unknown>>(
                coordinator, HEALTH_COMPONENT_SCHEMA, entity, 'health' as keyof Record<string, unknown>, 25, 'subtract'
            );
            effect.apply();
    ...
    describe('with typed components', () => {
        it('should work with typed components using new API', () => {
            ...
            const effect = new NumberModificationEffect<HealthComponent>(
                coordinator, HEALTH_COMPONENT_TYPED, entity, 'health', 25, 'subtract'
            );
            effect.apply();

            const updatedComponent = coordinator.getComponentFromEntity<HealthComponent>(HEALTH_COMPONENT_TYPED, entity);
            expect(updatedComponent?.health).toBe(75);
            expect(updatedComponent?.maxHealth).toBe(100);
        });
        ...
        it('should provide type safety for typed components', () => {
            ...
            const effect = new NumberModificationEffect<HealthComponent>(
                coordinator, HEALTH_COMPONENT_TYPED, entity, 'health', 10, 'add'
            );
            effect.apply();
    ...
    describe('automatic detection', () => {
        it('should prefer schema-based approach when both are possible', () => {
            ...
            const effect = new NumberModificationEffect<Record<string, unknown>>(
                coordinator, HEALTH_COMPONENT_SCHEMA, entity, 'health' as keyof Record<string, unknown>, 20, 'subtract'
            );
            effect.apply();
        ...
        it('should fall back to type-based approach when no schema exists', () => {
            ...
            const effect = new NumberModificationEffect<HealthComponent>(
                coordinator, HEALTH_COMPONENT_TYPED, entity, 'health', 30, 'subtract'
            );
            effect.apply();
    ...
    describe('backward compatibility', () => {
        it('should maintain backward compatibility with old API', () => {
            ...
            const effect = new NumberModificationEffect(
                coordinator, 15, HEALTH_COMPONENT_SCHEMA, entity, 'health', 'add'
            );
            effect.apply();
