...
describe('createHexGrid', () => {
    ...
    it('should create a grid entity with HexGridComponent', () => {
        const gridEntity = createHexGrid(
            coordinator,
            4,
            3,
            'test-hex-grid',
        ...
        const gridComponent =
            coordinator.getComponentFromEntity<HexGridComponent>(
                HEX_GRID_COMPONENT,
                gridEntity
            );
    ...
    it('should create correct number of cell zone entities', () => {
        ...
        const cellEntities = allEntities.filter(entity => {
            const locationComponent =
                coordinator.getComponentFromEntity<HexGridLocationComponent>(
                    HEX_GRID_LOCATION_COMPONENT,
                    entity
                );
            return (
        ...
        for (let q = 0; q < 2; q++) {
            for (let r = 0; r < 3; r++) {
                ...
                const cellEntity = cellEntities.find(entity => {
                    ...
                    return (
                        locationComponent?.q === q && locationComponent?.r === r
                    );
                });
    ...
    it('should register components if not already registered', () => {
        ...
        const gridComponentType =
            coordinator.getComponentType(HEX_GRID_COMPONENT);
...
describe('HexGridSystem', () => {
    ...
    describe('constructor', () => {
        ...
        it('should register components and system', () => {
            ...
            expect(
                newCoordinator.getComponentType(HEX_GRID_COMPONENT)
            ).not.toBeNull();
            expect(
                newCoordinator.getComponentType(HEX_GRID_LOCATION_COMPONENT)
            ).not.toBeNull();
            expect(
            ...
            const retrievedSystem =
                newCoordinator.getSystem<HexGridSystem>(HEX_GRID_SYSTEM);
