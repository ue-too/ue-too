...
describe('MoveEntityToZoneEffect - Ordered Zone', () => {
    ...
    describe('Moving entity to ordered zone - top', () => {
        beforeEach(() => {
            ...
            coordinator.addComponentToEntity(ZONE_COMPONENT, orderedZoneEntity, orderedZoneComponent);
        });

        it('should add entity to top of empty ordered zone', () => {
            const entity = coordinator.createEntity();
            const locationComponent: LocationComponent = { location: otherZoneEntity, sortIndex: 0 };
            coordinator.addComponentToEntity(LOCATION_COMPONENT, entity, locationComponent);

            const effect = new MoveEntityToZoneEffect(coordinator, entity, orderedZoneEntity, 'top');
            effect.apply();

            const updatedLocation = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, entity);
        ...
        it('should add entity to top and shift existing entities', () => {
            ...
            const location1: LocationComponent = { location: orderedZoneEntity, sortIndex: 0 };
            const location2: LocationComponent = { location: orderedZoneEntity, sortIndex: 1 };
            const location3: LocationComponent = { location: orderedZoneEntity, sortIndex: 2 };

            coordinator.addComponentToEntity(LOCATION_COMPONENT, existingEntity1, location1);
            coordinator.addComponentToEntity(LOCATION_COMPONENT, existingEntity2, location2);
            coordinator.addComponentToEntity(LOCATION_COMPONENT, existingEntity3, location3);
            ...
            const effect = new MoveEntityToZoneEffect(coordinator, newEntity, orderedZoneEntity, 'top');
            effect.apply();

            // Verify new entity is at top (sortIndex 0)
            const newEntityLocation = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, newEntity);
        ...
        it('should handle multiple entities added to top in sequence', () => {
            ...
            new MoveEntityToZoneEffect(coordinator, entity1, orderedZoneEntity, 'top').apply();
            new MoveEntityToZoneEffect(coordinator, entity2, orderedZoneEntity, 'top').apply();
            new MoveEntityToZoneEffect(coordinator, entity3, orderedZoneEntity, 'top').apply();
    ...
    describe('Moving entity to ordered zone - bottom', () => {
        ...
        it('should add entity to bottom of empty ordered zone', () => {
            ...
            const effect = new MoveEntityToZoneEffect(coordinator, entity, orderedZoneEntity, 'bottom');
            effect.apply();
        ...
        it('should add entity to bottom without shifting existing entities', () => {
            ...
            const effect = new MoveEntityToZoneEffect(coordinator, newEntity, orderedZoneEntity, 'bottom');
            effect.apply();
            ...
            const entitiesInZone = locationSystem.getEntitiesInZone(orderedZoneEntity);
            expect(entitiesInZone).toEqual([existingEntity1, existingEntity2, existingEntity3, newEntity]);
        });
        ...
        it('should handle multiple entities added to bottom in sequence', () => {
            ...
            new MoveEntityToZoneEffect(coordinator, entity1, orderedZoneEntity, 'bottom').apply();
            new MoveEntityToZoneEffect(coordinator, entity2, orderedZoneEntity, 'bottom').apply();
            new MoveEntityToZoneEffect(coordinator, entity3, orderedZoneEntity, 'bottom').apply();
    ...
    describe('Edge cases for ordered zones', () => {
        ...
        it('should not move entity if target is not a zone', () => {
            ...
            const effect = new MoveEntityToZoneEffect(coordinator, entity, nonZoneEntity, 'top');
            effect.apply();
        ...
        it('should not move entity if entity is already in the target zone', () => {
            const entity = coordinator.createEntity();
            const locationComponent: LocationComponent = { location: orderedZoneEntity, sortIndex: 0 };
            coordinator.addComponentToEntity(LOCATION_COMPONENT, entity, locationComponent);
        ...
        it('should handle moving entity from one ordered zone to another', () => {
            ...
            coordinator.addComponentToEntity(ZONE_COMPONENT, otherOrderedZone, otherOrderedZoneComponent);
            ...
            const location1: LocationComponent = { location: otherOrderedZone, sortIndex: 0 };
            const location2: LocationComponent = { location: otherOrderedZone, sortIndex: 1 };
            const location3: LocationComponent = { location: otherOrderedZone, sortIndex: 2 };
            ...
            const effect = new MoveEntityToZoneEffect(coordinator, entity2, orderedZoneEntity, 'top');
            effect.apply();

            // Verify entity2 is now in orderedZoneEntity
            const entity2Location = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, entity2);
            ...
            const entity1Location = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, entity1);
            const entity3Location = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, entity3);
            ...
            const entitiesInOrderedZone = locationSystem.getEntitiesInZone(orderedZoneEntity);
            expect(entitiesInOrderedZone).toEqual([entity2]);

            const entitiesInOtherZone = locationSystem.getEntitiesInZone(otherOrderedZone);
        ...
        it('should handle zone with disorganized sort indices', () => {
            ...
            const location1: LocationComponent = { location: orderedZoneEntity, sortIndex: 5 };
            const location2: LocationComponent = { location: orderedZoneEntity, sortIndex: 2 };
            const location3: LocationComponent = { location: orderedZoneEntity, sortIndex: 10 };
            ...
            const sortIndices = entitiesInZone.map(entity => {
                const loc = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, entity);
