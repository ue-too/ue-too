import { Coordinator, Entity, createComponentName, ComponentSchema, createGlobalComponentName, deserializeComponentSchema, SerializedComponentSchema } from "@ue-too/ecs";
...
describe('NumberModificationEffect', () => {
    ...
    describe('apply', () => {
        ...
        it('should handle zero as a valid field value', () => {
            ...
            coordinator.registerComponentWithSchema(schema);
            const component = coordinator.createComponentFromSchema(HEALTH_COMPONENT, { health: 0, score: 0 });
            coordinator.addComponentToEntityWithSchema(HEALTH_COMPONENT, entity, component);

            // Add to zero value
            const addEffect = new NumberModificationEffect(coordinator, 50, HEALTH_COMPONENT, entity, 'health', 'add');
            addEffect.apply();

            // Subtract from zero value (should go negative)
            const subtractEffect = new NumberModificationEffect(coordinator, 10, HEALTH_COMPONENT, entity, 'score', 'subtract');
            subtractEffect.apply();

            // Set to zero
            const setEffect = new NumberModificationEffect(coordinator, 0, HEALTH_COMPONENT, entity, 'health', 'set');
            setEffect.apply();
        ...
        it('should work with schema loaded from JSON file', () => {
            ...
            const component = coordinator.createComponentFromSchema(PLAYER_STATS_COMPONENT, {
                health: 100,
                maxHealth: 100,
                mana: 50,
                level: 5,
            ...
            coordinator.addComponentToEntityWithSchema(PLAYER_STATS_COMPONENT, entity, component);

            // Test subtract operation on health
            const healthEffect = new NumberModificationEffect(
                coordinator, 25, PLAYER_STATS_COMPONENT, entity, 'health', 'subtract'
            );
            healthEffect.apply();

            // Test add operation on mana
            const manaEffect = new NumberModificationEffect(
                coordinator, 10, PLAYER_STATS_COMPONENT, entity, 'mana', 'add'
            );
            manaEffect.apply();

            // Test set operation on experience
            const expEffect = new NumberModificationEffect(
                coordinator, 500, PLAYER_STATS_COMPONENT, entity, 'experience', 'set'
            );
            expEffect.apply();

            // Verify all modifications
            const updatedComponent = coordinator.getComponentFromEntity<Record<string, unknown>>(PLAYER_STATS_COMPONENT, entity);
        ...
        it('should work with schema loaded from JSON file using multiple operations', () => {
            ...
            const effects = [
                new NumberModificationEffect(coordinator, 20, PLAYER_STATS_COMPONENT, entity, 'health', 'subtract'),
                new NumberModificationEffect(coordinator, 15, PLAYER_STATS_COMPONENT, entity, 'health', 'subtract'),
                new NumberModificationEffect(coordinator, 5, PLAYER_STATS_COMPONENT, entity, 'mana', 'add'),
                new NumberModificationEffect(coordinator, 100, PLAYER_STATS_COMPONENT, entity, 'experience', 'add'),
                new NumberModificationEffect(coordinator, 1, PLAYER_STATS_COMPONENT, entity, 'level', 'add')
            ];
