...
describe('createOrthoGrid', () => {
    ...
    it('should create a grid entity with OrthoGridComponent', () => {
        ...
        const gridComponent =
            coordinator.getComponentFromEntity<OrthoGridComponent>(
                ORTHO_GRID_COMPONENT,
                gridEntity
            );
    ...
    it('should create correct number of cell zone entities', () => {
        const rows = 3;
        const columns = 4;
        const gridEntity = createOrthoGrid(
            coordinator,
            rows,
            columns,
            'test-grid'
        ...
        const cellEntities = allEntities.filter(entity => {
            const locationComponent =
                coordinator.getComponentFromEntity<OrthoGridLocationComponent>(
                    ORTHO_GRID_LOCATION_COMPONENT,
                    entity
                );
            return (
                locationComponent !== null &&
                locationComponent.grid === gridEntity
            );
        });
        ...
        for (let row = 0; row < 2; row++) {
            for (let column = 0; column < 3; column++) {
                ...
                const cellEntity = cellEntities.find(entity => {
                    ...
                    return (
                        locationComponent?.row === row &&
                        locationComponent?.column === column
                    );
                });
    ...
    it('should register components if not already registered', () => {
        ...
        const gridComponentType =
            coordinator.getComponentType(ORTHO_GRID_COMPONENT);
        const locationComponentType = coordinator.getComponentType(
            ORTHO_GRID_LOCATION_COMPONENT
        );
    ...
    it('should handle different grid names', () => {
        ...
        const grid1Cells = allEntitiesBefore.filter(entity => {
            ...
            return (
                locationComponent !== null && locationComponent.grid === grid1
            );
        });
        ...
        const grid2Cells = allEntitiesAfter.filter(entity => {
            ...
            return (
                locationComponent !== null && locationComponent.grid === grid2
            );
        });
...
describe('OrthoGridSystem', () => {
    ...
    describe('constructor', () => {
        ...
        it('should register components and system', () => {
            ...
            expect(
                newCoordinator.getComponentType(ORTHO_GRID_COMPONENT)
            ).not.toBeNull();
            expect(
                newCoordinator.getComponentType(ORTHO_GRID_LOCATION_COMPONENT)
            ).not.toBeNull();
            expect(
                newCoordinator.getComponentType(ZONE_COMPONENT)
            ).not.toBeNull();

            // Verify system is registered
            const retrievedSystem =
                newCoordinator.getSystem<OrthoGridSystem>(ORTHO_GRID_SYSTEM);
        ...
        it('should automatically add entities with required components to system', () => {
            ...
            newCoordinator.addComponentToEntity(
                ORTHO_GRID_LOCATION_COMPONENT,
                cellEntity,
                {
                    grid: grid,
    ...
    describe('getEntireGridEntities', () => {
        ...
        it('should handle different grid sizes', () => {
            ...
            new LocationSystem(newCoordinator);
            const newGridSystem = new OrthoGridSystem(newCoordinator);
            const smallGrid = createOrthoGrid(
                newCoordinator,
                2,
                2,
                'small-grid'
    ...
    describe('addEntityToGridCell', () => {
        ...
        it('should add entity to grid cell zone', () => {
            ...
            const entitiesInZone = locationSystem.getEntitiesInZone(
                cellEntity!
            );
        ...
        it('should do nothing if LocationSystem is not available', () => {
            ...
            const originalLocation =
                newCoordinator.getComponentFromEntity<LocationComponent>(
                    LOCATION_COMPONENT,
                    entity
                );
