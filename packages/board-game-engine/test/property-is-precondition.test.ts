...
describe('PropertyIsPrecondition', () => {
    ...
    let entity: Entity;
    const PLAYER_COMPONENT_SCHEMA = createGlobalComponentName(
        'PlayerComponentSchema'
    );
    const PLAYER_COMPONENT_TYPED = createGlobalComponentName(
        'PlayerComponentTyped'
    );
    ...
    describe('with typed components', () => {
        ...
        describe('number values', () => {
            it('should return true when number property matches', () => {
                const precondition =
                    new PropertyIsPrecondition<PlayerComponent>(
                        coordinator,
                        PLAYER_COMPONENT_TYPED,
                        entity,
                        'level',
            ...
            it('should return false when property does not exist', () => {
                const precondition =
                    new PropertyIsPrecondition<PlayerComponent>(
                        ...
                        PLAYER_COMPONENT_TYPED,
                        entity,
                        'nonExistentField' as keyof PlayerComponent,
                        'value'
                    );
                expect(precondition.check()).toBe(false);
            });
    ...
    describe('unified behavior', () => {
        it('should use schema-based approach when schema exists', () => {
            const schema: ComponentSchema = {
                componentName: PLAYER_COMPONENT_SCHEMA,
                fields: [{ name: 'level', type: 'number', defaultValue: 10 }],
            };
            coordinator.registerComponentWithSchema(schema);
            const component = coordinator.createComponentFromSchema(
                PLAYER_COMPONENT_SCHEMA,
            ...
            >(
                ...
                PLAYER_COMPONENT_SCHEMA,
                entity,
                'level' as keyof Record<string, unknown>,
                10
            );
            expect(precondition.check()).toBe(true);
        });
