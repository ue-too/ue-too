...
type AdvanceAtTWithLengthOutofCurveRes =
    | AdvanceAtWithLengthBeforeCurveRes
    | AdvanceAtWithLengthAfterCurveRes;

type AdvanceAtTWithLengthRes =
    | AdvanceAtTWithLengthWithinCurveRes
    | AdvanceAtTWithLengthOutofCurveRes;
...
export class BCurve {
    ...
    private arcLengthLUT: ArcLengthLUT = {
        controlPoints: [],
        arcLengthLUT: [],
    };
    ...
    constructor(controlPoints: Point[]) {
        ...
        this.dControlPoints = this.getDerivativeControlPoints(
            this.controlPoints
        );
    ...
    private validateTVal(tVal: number) {
        if (tVal > 1 || tVal < 0) {
            throw new TValOutofBoundError(
                'tVal is greater than 1 or less than 0'
            );
        }
    }
    ...
    public compute(tVal: number): Point {
        ...
        while (points.length > 1) {
            ...
            for (let index = 0; index < lowerLevelPoints.length; index++) {
                lowerLevelPoints[index] = PointCal.addVector(
                    PointCal.multiplyVectorByScalar(points[index], 1 - tVal),
                    PointCal.multiplyVectorByScalar(points[index + 1], tVal)
                );
            }
    ...
    public get(tVal: number): Point {
        ...
        if (this.controlPoints.length == 3) {
            let firstTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[0],
                (1 - tVal) * (1 - tVal)
            );
            let secondTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[1],
                2 * (1 - tVal) * tVal
            );
            let thirdTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[2],
                tVal * tVal
            );
            let res = PointCal.addVector(
                PointCal.addVector(firstTerm, secondTerm),
                thirdTerm
            );
            return res;
        }
        if (this.controlPoints.length == 4) {
            let firstTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[0],
                (1 - tVal) * (1 - tVal) * (1 - tVal)
            );
            let secondTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[1],
                3 * (1 - tVal) * (1 - tVal) * tVal
            );
            let thirdTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[2],
                3 * (1 - tVal) * tVal * tVal
            );
            let forthTerm = PointCal.multiplyVectorByScalar(
                this.controlPoints[3],
                tVal * tVal * tVal
            );
            let res = PointCal.addVector(
                PointCal.addVector(firstTerm, secondTerm),
                PointCal.addVector(thirdTerm, forthTerm)
            );
            return res;
        }
    ...
    public derivativeNormalized(tVal: number): Point {
        return PointCal.unitVector(
            computeWithControlPoints(tVal, this.dControlPoints)
        );
    }

    public getArcLengthLUT(steps: number = 50): {
        controlPoints: Point[];
        arcLengthLUT: { tVal: number; length: number }[];
    } {
        ...
        if (
            ...
            this.arcLengthLUT.arcLengthLUT.length === 0
        ) {
    ...
    public split(tVal: number): [Point[], Point[]] {
        ...
        if (this.controlPoints.length == 3) {
            ...
            let newControlPoint2 = PointCal.subVector(
                PointCal.multiplyVectorByScalar(this.controlPoints[1], tVal),
                PointCal.multiplyVectorByScalar(this.controlPoints[0], tVal - 1)
            );
            let newControlPoint3 = PointCal.subVector(
                PointCal.multiplyVectorByScalar(
                    this.controlPoints[2],
                    tVal * tVal
                ),
            ...
            newControlPoint3 = PointCal.addVector(
                newControlPoint3,
                PointCal.multiplyVectorByScalar(
                    this.controlPoints[0],
                    (tVal - 1) * (tVal - 1)
            ...
            let newControlPoint4 = PointCal.subVector(
                PointCal.multiplyVectorByScalar(this.controlPoints[2], tVal),
                PointCal.multiplyVectorByScalar(this.controlPoints[1], tVal - 1)
            );
            let newControlPoint5 = this.controlPoints[2];
            return [
                [newControlPoint1, newControlPoint2, newControlPoint3],
                [newControlPoint3, newControlPoint4, newControlPoint5],
            ];
        }
        let newControlPoint1 = this.controlPoints[0];
        let newControlPoint2 = PointCal.subVector(
        ...
        let newControlPoint3 = PointCal.addVector(
            PointCal.multiplyVectorByScalar(this.controlPoints[2], tVal * tVal),
            PointCal.addVector(
                PointCal.multiplyVectorByScalar(
                    this.controlPoints[1],
                    -(2 * tVal * (tVal - 1))
        ...
        let term1 = PointCal.multiplyVectorByScalar(
            this.controlPoints[3],
            tVal * tVal * tVal
        );
        let term2 = PointCal.multiplyVectorByScalar(
            this.controlPoints[2],
            -(3 * tVal * tVal * (tVal - 1))
        );
        let term3 = PointCal.multiplyVectorByScalar(
            this.controlPoints[1],
            3 * tVal * (tVal - 1) * (tVal - 1)
        );
        let term4 = PointCal.multiplyVectorByScalar(
            this.controlPoints[0],
            -((tVal - 1) * (tVal - 1) * (tVal - 1))
        );
        let newControlPoint4 = PointCal.addVector(
            term4,
            PointCal.addVector(term3, PointCal.addVector(term1, term2))
        );
        let newControlPoint5 = PointCal.addVector(
            PointCal.addVector(
                PointCal.multiplyVectorByScalar(
                    this.controlPoints[3],
                    tVal * tVal
        ...
        let newControlPoint6 = PointCal.addVector(
            PointCal.multiplyVectorByScalar(this.controlPoints[3], tVal),
            PointCal.multiplyVectorByScalar(this.controlPoints[2], -(tVal - 1))
        );
    ...
    splitAndTakeMidCurve(tVal: number, tVal2: number): BCurve {
        const [firstSplit, secondSplit, thirdSplit] = this.splitIn3Curves(
            tVal,
            tVal2
        );
        return secondSplit;
    ...
    getProjection(point: Point) {
        ...
        LUT.forEach((curvePoint, index) => {
            const curDistance = PointCal.distanceBetweenPoints(
                curvePoint.point,
                point
            );
        ...
        while (low < high && high - low > threshold) {
            ...
            if (lowMidMid <= 1 && lowMidMid >= 0) {
                let curDist = PointCal.distanceBetweenPoints(
                    this.get(lowMidMid),
                    point
                );
                if (curDist < distance) {
    ...
    public findArcStartingAt(errorThreshold: number, low: number) {
        ...
        while (true) {
            ...
            if (
                !fitArcRes.exists ||
                fitArcRes.center == null ||
                fitArcRes.radius == null
            ) {
                return null;
            }
            const n = high - mid;
            ...
            const checkRadius = PointCal.distanceBetweenPoints(
                checkPoint1,
                fitArcRes.center
            );
            const checkRadius2 = PointCal.distanceBetweenPoints(
                checkPoint2,
                fitArcRes.center
            );
            if (
                Math.abs(checkRadius - fitArcRes.radius) > errorThreshold ||
                Math.abs(checkRadius2 - fitArcRes.radius) > errorThreshold
            ) {
                // arc is bad
                ...
                prevArc.good = true;
                if (
                    fitArcRes.startPoint !== undefined &&
                    fitArcRes.endPoint !== undefined
                ) {
                    prevArc.arc = {
    ...
    public fitArc(
        ...
        const centerX =
            (1 / 2) * (this.determinant3by3(M12) / this.determinant3by3(M11));
        const centerY =
    ...
    public curvature(tVal: number): number {
        ...
        const secondDerivative = computeWithControlPoints(
            tVal,
            this.getDerivativeControlPoints(this.dControlPoints)
        );
        const numerator =
            derivative.x * secondDerivative.y -
    ...
    secondDerivative(tVal: number): Point {
        return computeWithControlPoints(
            tVal,
            this.getDerivativeControlPoints(this.dControlPoints)
        );
    }
    ...
    public getDerivativeCoefficients(): Point[] {
        return this.getCoefficientOfTTermsWithControlPoints(
            this.dControlPoints
        );
    }

    public getCoefficientOfTTermsWithControlPoints(
        controlPoints: Point[]
    ): Point[] {
        const terms: Point[] = [];
        let matrix: number[][] = [];
    ...
    getControlPointsAlignedWithXAxis() {
        const alignedAxis = PointCal.unitVectorFromA2B(
            this.controlPoints[0],
        ...
        for (let index = 1; index < this.controlPoints.length; index++) {
            const vector = PointCal.subVector(
    ...
    getExtrema(): { x: number[]; y: number[] } {
        ...
        const xRoots = solveCubic(
            xCoefficients[0],
            xCoefficients[1],
            xCoefficients[2],
            xCoefficients[3]
        );
        const yRoots = solveCubic(
            yCoefficients[0],
            yCoefficients[1],
            yCoefficients[2],
            yCoefficients[3]
        ...
        if (derivativeCoefficients.length >= 3) {
            ...
            const secondDerivativeCoefficients =
                this.getCoefficientOfTTermsWithControlPoints(
                    this.getDerivativeControlPoints(this.dControlPoints)
                );
            ...
            const secondXRoots = solveCubic(
                xCoefficients[0],
                xCoefficients[1],
                xCoefficients[2],
                xCoefficients[3]
            );
            const secondYRoots = solveCubic(
                yCoefficients[0],
                yCoefficients[1],
                yCoefficients[2],
                yCoefficients[3]
        ...
        for (let index = 0; index < this.controlPoints.length; index++) {
            res.push(
                PointCal.rotatePoint(
                    PointCal.addVector(this.controlPoints[index], translation),
                    rotationAngle
                )
            );
        }
    ...
    getLineIntersections(line: Line): number[] {
        ...
        const res: number[] = [];
        const alignedControlPoints = this.translateRotateControlPoints(
            translationRotation.translation,
            translationRotation.rotationAngle
        );
        const coefficients =
            this.getCoefficientOfTTermsWithControlPoints(alignedControlPoints);
        let yCoefficients = [0, 0, 0, 0];
    ...
    getSelfIntersections(): { selfT: number; otherT: number }[] {
        const [subCurveControlPoints1, subCurveControlPoints2] =
            this.split(0.5);
    ...
    getCircleIntersections(
        circleCenter: Point,
        circleRadius: number
    ): { intersection: Point; tVal: number }[] {
        const LUT = this.getLUTWithTVal(500);
        ...
        LUT.forEach((curvePoint, index) => {
            let curDistanceError = Math.abs(
                PointCal.distanceBetweenPoints(circleCenter, curvePoint.point)
            );
        ...
        let indices: number[] = [];
        while (++count < 25) {
            let i = this.findClosest(
                circleCenter.x,
                circleCenter.y,
                LUTD,
                circleRadius,
                5,
                LUTD[start - 2]?.distance,
                LUTD[start - 1]?.distance
            );
            if (i < start) break;
    ...
    findClosest(
        ...
        for (let index = 0, e = LUT.length; index < e; index++) {
            ...
            LUT[index].distance = Math.abs(
                PointCal.distanceBetweenPoints({ x: x, y: y }, p) - circleRadius
            );
    ...
    ): { selfT: number; otherT: number }[] {
        return getIntersectionsBetweenCurves(
            this,
            curve,
...
export function offset(
    ...
    t: number,
    d: number
): { c: Point; n: Point; x: number; y: number };
...
function curveIsLinear(curve: BCurve): boolean {
    ...
    const linear =
        alignedPoints.reduce((t, p) => t + Math.abs(p.y), 0) < baseLength / 50;
 ...
 * @category Utilities
 */
export function getIntersectionsBetweenCurves(
    curve: BCurve,
    curve2: BCurve,
    deduplicationTolerance: number = 0.01
): { selfT: number; otherT: number }[] {
    const threshold = 0.5;
    let pairs: {
    ...
    while (pairs.length > 0) {
        ...
        for (let index = 0; index < curLength; index++) {
            ...
            if (
                pair.curve1.curve.fullLength < threshold &&
                pair.curve2.curve.fullLength < threshold
            ) {
                finalRes.push({
            ...
            if (intersects) {
                let [subCurveControlPoints1, subCurveControlPoints2] =
                    pair.curve1.curve.split(0.5);
                let [subCurveControlPoints3, subCurveControlPoints4] =
                    pair.curve2.curve.split(0.5);
                pairs.push({
                    curve1: {
                        curve: new BCurve(subCurveControlPoints1),
                        startTVal: pair.curve1.startTVal,
                        endTVal:
                            pair.curve1.startTVal +
                            (pair.curve1.endTVal - pair.curve1.startTVal) * 0.5,
                    },
                    curve2: {
                        curve: new BCurve(subCurveControlPoints3),
                        startTVal: pair.curve2.startTVal,
                        endTVal:
                            pair.curve2.startTVal +
                            (pair.curve2.endTVal - pair.curve2.startTVal) * 0.5,
                    },
                });
    ...
    for (const intersection of finalRes) {
        ...
        for (const existing of tVals) {
            ...
            const selfTClose = approximately(
                intersection.tVal1,
                existing.selfT,
                deduplicationTolerance
            );
            const otherTClose = approximately(
                intersection.tVal2,
                existing.otherT,
                deduplicationTolerance
            );
            ...
            const selfTVeryClose = approximately(
                intersection.tVal1,
                existing.selfT,
                deduplicationTolerance * 10
            );
            const otherTVeryClose = approximately(
                intersection.tVal2,
                existing.otherT,
                deduplicationTolerance * 10
            );
            ...
            if (selfTVeryClose || otherTVeryClose) {
                ...
                const distance1 = PointCal.distanceBetweenPoints(
                    point1,
                    existingPoint1
                );
                const distance2 = PointCal.distanceBetweenPoints(
                    point2,
                    existingPoint2
                );
                ...
                if (
                    distance1 < deduplicationTolerance * 100 &&
                    distance2 < deduplicationTolerance * 100
                ) {
                    isDuplicate = true;
                    break;
                }
...
export function solveCubic(a: number, b: number, c: number, d: number) {
    ...
    if (Math.abs(p) < 1e-8) {
        ...
        if (Math.abs(D) < 1e-8) {
            ...
            // Only one real root and two complex roots
 ...
 * @category Utilities
 */
export function computeWithControlPoints(
    tVal: number,
    controlPoints: Point[]
): Point {
...
function translate(curve: BCurve, vector: Point, d1: number, d2: number) {
    ...
    const points = curve.getControlPoints();
    const d = points.map(
        (_, i: number) => (1 - i / order) * d1 + (i / order) * d2
    );
    return new BCurve(
        points.map((p, i) => ({
            x: p.x + d[i] * vector.x,
