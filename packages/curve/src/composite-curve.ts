...
export class ControlPoint {
    ...
    constructor(
        position: Point,
        leftHandle: HandlePoint,
        rightHandle: HandlePoint
    ) {
    ...
    setPosition(
        ...
        prevControlPoint: ControlPoint | undefined,
        nextControlPoint: ControlPoint | undefined
    ) {
        let diff = PointCal.subVector(destinationPosition, this.position);
        this.position = destinationPosition;
        this.leftHandle.position = PointCal.addVector(
            this.leftHandle.position,
            diff
        );
        this.rightHandle.position = PointCal.addVector(
            this.rightHandle.position,
            diff
        );
        if (this.leftHandle.type == 'VECTOR' && prevControlPoint) {
            let relativeVector = PointCal.subVector(
                prevControlPoint.getPosition(),
                this.position
            );
            relativeVector = PointCal.multiplyVectorByScalar(
                relativeVector,
                1 / 3
            );
            this.leftHandle.position = PointCal.addVector(
            ...
            if (this.rightHandle.type == 'ALIGNED') {
                ...
                let direction = PointCal.unitVectorFromA2B(
                    this.leftHandle.position,
                    this.position
                );
                this.rightHandle.position = PointCal.addVector(
                    this.position,
                    PointCal.multiplyVectorByScalar(direction, mag)
                );
            }
        }
        if (this.rightHandle.type == 'VECTOR' && nextControlPoint) {
            let relativeVector = PointCal.subVector(
                nextControlPoint.getPosition(),
                this.position
            );
            relativeVector = PointCal.multiplyVectorByScalar(
                relativeVector,
            ...
            if (this.leftHandle.type == 'ALIGNED') {
                let mag = PointCal.distanceBetweenPoints(
                    this.leftHandle.position,
                    this.position
                );
                let direction = PointCal.subVector(
                    this.position,
                    this.rightHandle.position
                );
                this.leftHandle.position = PointCal.addVector(
        ...
        if (
            prevControlPoint !== undefined &&
            prevControlPoint.getRightHandle().type == 'VECTOR'
        ) {
            let relativeVector = PointCal.subVector(
                this.position,
                prevControlPoint.getPosition()
            ...
            prevControlPoint.setRightHandlePosition(
                PointCal.addVector(
                    prevControlPoint.getPosition(),
                    relativeVector
                )
        ...
        if (
            nextControlPoint !== undefined &&
            nextControlPoint.getLeftHandle().type == 'VECTOR'
        ) {
            let relativeVector = PointCal.subVector(
                this.position,
                nextControlPoint.getPosition()
            ...
            nextControlPoint.setLeftHandlePosition(
                PointCal.addVector(
                    nextControlPoint.getPosition(),
                    relativeVector
                )
    ...
    setLeftHandleTypeVector(prevControlPoint: ControlPoint | undefined) {
        ...
        if (prevControlPoint == undefined) {
            relativeVector = PointCal.subVector(
                this.leftHandle.position,
                this.position
            );
        } else {
    ...
    setRightHandleTypeAligned() {
        ...
        if (this.leftHandle.type == 'VECTOR') {
            let direciton = PointCal.unitVectorFromA2B(
                this.leftHandle.position,
                this.position
            );
            let mag = PointCal.distanceBetweenPoints(
            ...
            this.rightHandle.position = PointCal.addVector(
                this.position,
                PointCal.multiplyVectorByScalar(direciton, mag)
            );
        }
    }
        ...
        switch (leftHandleType) {
            case 'ALIGNED':
                if (this.rightHandle.type == 'VECTOR') {
                    ...
                    let rightHandleDiff = PointCal.unitVectorFromA2B(
                        this.rightHandle.position,
                        this.position
                    );
                    let resMag = PointCal.dotProduct(diff, rightHandleDiff);
                    let res = PointCal.multiplyVectorByScalar(
                        rightHandleDiff,
                        resMag
                    );
                    this.leftHandle.position = PointCal.addVector(
    ...
    setRightHandlePosition(destPos: Point) {
        ...
        switch (rightHandleType) {
            case 'ALIGNED':
                if (this.leftHandle.type == 'VECTOR') {
                    let diff = PointCal.subVector(destPos, this.position);
                    let leftHandleDiff = PointCal.unitVectorFromA2B(
                        this.leftHandle.position,
                        this.position
                    );
...
export class CompositeBCurve {
    ...
    appendControlPoint(position: Point) {
        let leftHandlePosition = PointCal.addVector(position, {
            x: -100,
            y: 0,
        });
        let rightHandlePosition = PointCal.addVector(position, {
            x: 100,
            y: 0,
        });
        ...
        let newControlPoint = new ControlPoint(
            position,
            leftHandlePoint,
            rightHandlePoint
        );
    ...
    setLeftHandlePositionOfControlPoint(
        ...
        if (
            controlPointIndex >= this.controlPoints.length ||
            controlPointIndex < 0
        ) {
            return;
        }
    ...
    setPositionOfControlPoint(controlPointIndex: number, destPos: Point) {
        ...
        this.controlPoints[controlPointIndex].setPosition(
            destPos,
            prevControlPoint,
            nextControlPoint
        );
