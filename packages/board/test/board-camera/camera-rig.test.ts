...
describe('camera rig', () => {
    it('should zoom at viewport anchor with the correct before and after zoom', () => {
        ...
        const viewportAnchorInWorldBeforeZoom =
            cameraRig.camera.convertFromViewPort2WorldSpace(viewportAnchor);
        cameraRig.zoomByAt(1, viewportAnchor);
        const viewportAnchorInWorldAfterZoom =
            cameraRig.camera.convertFromViewPort2WorldSpace(viewportAnchor);
        expect(viewportAnchorInWorldAfterZoom.x).toBe(
            viewportAnchorInWorldBeforeZoom.x
        );
        expect(viewportAnchorInWorldAfterZoom.y).toBe(
            viewportAnchorInWorldBeforeZoom.y
        );
        expect(viewportAnchorInWorldAfterZoom.z).toBe(
            viewportAnchorInWorldBeforeZoom.z
        );
    });
    ...
    it('should zoom by at world anchor with the anchor appear at the same position in view port', () => {
        ...
        const viewportAnchor =
            cameraRig.camera.convertFromWorld2ViewPort(worldAnchor);
        cameraRig.zoomByAtWorld(1, worldAnchor);
        const viewportAnchorAfterZoom =
            cameraRig.camera.convertFromWorld2ViewPort(worldAnchor);
    ...
    it('should restrict translation to only move in relative x axis (horizontal direction in viewport) if configured to do so', () => {
        ...
        const horizontalDirectionInViewportInWorldSpace = PointCal.rotatePoint(
            { x: 1, y: 0 },
            cameraRig.camera.rotation
        );
        cameraRig.configure({ restrictRelativeXTranslation: true });
        cameraRig.panByViewPort({ x: 100, y: 100 });
        const res = PointCal.dotProduct(
            cameraRig.camera.position,
            horizontalDirectionInViewportInWorldSpace
        );
    ...
    it('should restrict translation to only move in relative y axis (vertical direction in viewport) if configured to do so', () => {
        const cameraRig = createDefaultCameraRig(new DefaultBoardCamera());
        cameraRig.rotateBy((45 * Math.PI) / 180);
        const verticalDirectionInViewportInWorldSpace = PointCal.rotatePoint(
            { x: 0, y: 1 },
            cameraRig.camera.rotation
        );
        ...
        const res = PointCal.dotProduct(
            cameraRig.camera.position,
            verticalDirectionInViewportInWorldSpace
        );
        expect(res).toBeCloseTo(0);
    });
    ...
    describe('rotation with limitEntireViewPort', () => {
        it('should adjust camera position when rotateBy causes viewport to exceed boundaries', () => {
            ...
            expect(cameraRig.camera.position.x).not.toBe(
                positionBeforeRotation.x
            );
            expect(cameraRig.camera.position.y).not.toBe(
                positionBeforeRotation.y
            );
            ...
            expect(viewportCorners.top.left.x).toBeGreaterThanOrEqual(
                boundaries.min.x
            );
            expect(viewportCorners.top.left.y).toBeGreaterThanOrEqual(
                boundaries.min.y
            );
            expect(viewportCorners.top.right.x).toBeLessThanOrEqual(
                boundaries.max.x
            );
            expect(viewportCorners.top.right.y).toBeGreaterThanOrEqual(
                boundaries.min.y
            );
            expect(viewportCorners.bottom.left.x).toBeGreaterThanOrEqual(
                boundaries.min.x
            );
            expect(viewportCorners.bottom.left.y).toBeLessThanOrEqual(
                boundaries.max.y
            );
            expect(viewportCorners.bottom.right.x).toBeLessThanOrEqual(
                boundaries.max.x
            );
            expect(viewportCorners.bottom.right.y).toBeLessThanOrEqual(
                boundaries.max.y
            );
        });
