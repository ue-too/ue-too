...
import {
    ...
    translationHeightOf,
    translationWidthOf,
} from '../../src/camera/utils/position';
import {
    RotationBoundary,
    RotationLimits,
    clampRotation,
...
} from '../../src/camera/utils/rotation';
...
describe('withinBoundaries', () => {
    ...
    test('should still work if only one boundary is defined', () => {
        expect(
            withinBoundaries({ x: 0, y: 0 }, { min: { x: -1, y: -1 } })
        ).toBe(true);
        expect(withinBoundaries({ x: 0, y: 0 }, { max: { x: 1, y: 1 } })).toBe(
            true
        );
        expect(withinBoundaries({ x: 0, y: 0 }, { min: { x: 1, y: 1 } })).toBe(
            false
        );
        expect(
            withinBoundaries({ x: 0, y: 0 }, { max: { x: -1, y: -1 } })
        ).toBe(false);
    });
    ...
    test('should still work if only one axis is defined', () => {
        ...
        expect(withinBoundaries({ x: 0, y: 0 }, { max: { x: -1 } })).toBe(
            false
        );
    });
});
...
describe('rotation boundaries testing', () => {
    test('should return true if the rotation is within the boundaries', () => {
        const rotationBoundaries: RotationBoundary = {
            start: 0,
            end: Math.PI,
        ...
        expect(rotationWithinBoundary(Math.PI / 2, rotationBoundaries)).toBe(
            true
        );
    });
    ...
    test('should return false if the rotation is outside the boundaries', () => {
        ...
        expect(
            rotationWithinBoundary((Math.PI * 3) / 2, rotationBoundaries)
        ).toBe(false);
    });

    test('should still work if the boundaries cross the 0 degree mark', () => {
        const rotationBoundaries: RotationBoundary = {
            start: ((360 - 45) * Math.PI) / 180,
            end: (45 * Math.PI) / 180,
            positiveDirection: true,
            startAsTieBreaker: true,
        };
        expect(rotationWithinBoundary(0, rotationBoundaries)).toBe(true);
        expect(
            rotationWithinBoundary((50 * Math.PI) / 180, rotationBoundaries)
        ).toBe(false);
        expect(
            rotationWithinBoundary((330 * Math.PI) / 180, rotationBoundaries)
        ).toBe(true);
        expect(
            rotationWithinBoundary((300 * Math.PI) / 180, rotationBoundaries)
        ).toBe(false);
        expect(
            rotationWithinBoundary((-50 * Math.PI) / 180, rotationBoundaries)
        ).toBe(false);
        expect(
            rotationWithinBoundary((315 * Math.PI) / 180, rotationBoundaries)
        ).toBe(true);
    });
    ...
    test('should return true for a full revolution boundary', () => {
        ...
        expect(
            rotationWithinBoundary(Math.PI * 2 - 1, rotationBoundaries)
        ).toBe(true);
    });
});
...
describe('rotationLimits', () => {
    ...
    test('should return false if the rotation is outside the boundaries', () => {
        ...
        expect(rotationWithinLimits((Math.PI * 3) / 2, rotationLimits)).toBe(
            false
        );
    });
    ...
    test('should still work if the boundaries cross the 0 degree mark', () => {
        ...
        expect(rotationWithinLimits(0, rotationLimits)).toBe(true);
        expect(rotationWithinLimits((50 * Math.PI) / 180, rotationLimits)).toBe(
            false
        );
        expect(
            rotationWithinLimits((330 * Math.PI) / 180, rotationLimits)
        ).toBe(true);
        expect(
            rotationWithinLimits((300 * Math.PI) / 180, rotationLimits)
        ).toBe(false);
        expect(
            rotationWithinLimits((-50 * Math.PI) / 180, rotationLimits)
        ).toBe(false);
        expect(
            rotationWithinLimits((315 * Math.PI) / 180, rotationLimits)
        ).toBe(true);
    });
    ...
    test('should return true for a full revolution limit', () => {
        ...
        expect(rotationWithinLimits(0, rotationLimits)).toBe(true);
        expect(rotationWithinLimits(Math.PI * 2, rotationLimits)).toBe(true);
        expect(rotationWithinLimits(Math.PI * 2 + 1, rotationLimits)).toBe(
            true
        );
        expect(rotationWithinLimits(Math.PI * 2 - 1, rotationLimits)).toBe(
            true
...
describe('normalizeAngleZero2TwoPI', () => {
    ...
    test('should return the normalized angle if it is not between 0 and 2PI', () => {
        expect(normalizeAngleZero2TwoPI((-45 * Math.PI) / 180)).toBeCloseTo(
            (315 * Math.PI) / 180
        );
        expect(normalizeAngleZero2TwoPI(-Math.PI)).toBe(Math.PI);
        expect(normalizeAngleZero2TwoPI(-Math.PI * 2)).toBe(0);
...
describe('angle clamping', () => {
    ...
    test('counter-clockwise rotation limits crossing the 0 degree mark', () => {
        ...
        expect(clampRotation(0, limits)).toBe(0);
        expect(clampRotation((50 * Math.PI) / 180, limits)).toBe(
            (45 * Math.PI) / 180
        );
        expect(clampRotation((330 * Math.PI) / 180, limits)).toBeCloseTo(
            (330 * Math.PI) / 180
        );
        expect(clampRotation((300 * Math.PI) / 180, limits)).toBe(
            (315 * Math.PI) / 180
        );
        expect(clampRotation((-50 * Math.PI) / 180, limits)).toBe(
            (315 * Math.PI) / 180
...
describe('calculate the minimum angle span from an angle to another', () => {
    ...
    test('testing the angle span that rotating clockwise is smaller than rotating counter clockwise', () => {
        expect(angleSpan(0, (270 * Math.PI) / 180)).toBeCloseTo(
            (-90 * Math.PI) / 180
        );
    });
    ...
    test('testing the angle span that cross the 0 degree mark', () => {
        expect(
            angleSpan((22.5 * Math.PI) / 180, (-22.5 * Math.PI) / 180)
...
describe('coordinate conversion', () => {
    test('Convert point within camera view to world space', () => {
        const testRes = convert2WorldSpace(
            { x: 100, y: 100 },
            1000,
            1000,
            { x: 30, y: 50 },
        ...
        const testRes2 = convert2WorldSpace(
            { x: 100, y: 100 },
            1000,
            1000,
            { x: 10, y: 10 },
    ...
    test('Convert point within camera view to world space with the camera position as view port origin', () => {
        const testRes = convert2WorldSpaceAnchorAtCenter(
            { x: -400, y: -400 },
            { x: 30, y: 50 },
            10,
            (-45 * Math.PI) / 180
        ...
        expect(testRes.x).toBeCloseTo(30 - 800 / (Math.sqrt(2) * 10));
        expect(testRes.y).toBeCloseTo(50);
        const testRes2 = convert2WorldSpaceAnchorAtCenter(
            { x: -400, y: -400 },
            { x: 10, y: 10 },
            1,
            0
    ...
    test('Convert point within world space to camera view', () => {
        ...
        const testRes = invertFromWorldSpace(
            point,
            1000,
            1000,
            { x: 30, y: 50 },
        ...
        const testRes2 = invertFromWorldSpace(
            test2Point,
            1000,
            1000,
            { x: 30, y: 50 },
        ...
        const expectedRes = {
            x: 500 - 20 / Math.sqrt(2),
            y: 500 - 20 / Math.sqrt(2),
        };
    ...
    test('Convert point within world space to camera view with camera position as viewport origin', () => {
        const point = { x: 10, y: 30 };
        const testRes = convert2ViewPortSpaceAnchorAtCenter(
            point,
            { x: 30, y: 50 },
            1,
            0
        ...
        const test2Point = { x: 10, y: 50 };
        const testRes2 = convert2ViewPortSpaceAnchorAtCenter(
            test2Point,
            { x: 30, y: 50 },
            1,
            (-45 * Math.PI) / 180
    ...
    describe('coorindate conversion using transformation matrix', () => {
        test('should convert point within world space to camera view', () => {
            ...
            const transformationMatrix = transformationMatrixFromCamera(
                { x: 30, y: 50 },
                10,
                (-45 * Math.PI) / 180
            );
            const testRes = convert2WorldSpaceWithTransformationMatrix(
                point,
                transformationMatrix
            );
            expect(testRes.x).toBeCloseTo(30 - 800 / (Math.sqrt(2) * 10));
            expect(testRes.y).toBeCloseTo(50);
            const transformationMatrix2 = transformationMatrixFromCamera(
                { x: 10, y: 10 },
                1,
                0
            );
            const testRes2 = convert2WorldSpaceWithTransformationMatrix(
                point,
