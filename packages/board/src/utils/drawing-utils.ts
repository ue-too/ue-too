...
export function drawGrid(
    ...
    let leftRightDirection = PointCal.unitVectorFromA2B(
        topLeftCorner,
        topRightCorner
    );
    let topDownDirection = PointCal.unitVectorFromA2B(
        bottomLeftCorner,
        topLeftCorner
    );
    let width = PointCal.distanceBetweenPoints(topLeftCorner, topRightCorner);
    let height = PointCal.distanceBetweenPoints(
        topLeftCorner,
        bottomLeftCorner
    );
    ...
    let minHorizontalSmallTick =
        Math.ceil(topLeftCorner.x / subDivisor) * subDivisor;
    let maxHorizontalSmallTick =
        Math.floor(topRightCorner.x / subDivisor) * subDivisor;
    let minVerticalSmallTick = alignCoordinateSystem
        ? Math.floor(topLeftCorner.y / subDivisor) * subDivisor
        : Math.ceil(bottomLeftCorner.y / subDivisor) * subDivisor;
    let maxVerticalSmallTick = alignCoordinateSystem
        ? Math.ceil(bottomLeftCorner.y / subDivisor) * subDivisor
        : Math.floor(topLeftCorner.y / subDivisor) * subDivisor;

    // vertical lines
    for (
        let i = minHorizontalSmallTick;
        i <= maxHorizontalSmallTick;
        i += subDivisor
    ) {
        const startPoint = PointCal.addVector(
            { x: i, y: topLeftCorner.y },
            PointCal.multiplyVectorByScalar(leftRightDirection, subDivisor)
        );
        const endPoint = PointCal.addVector(
            { x: i, y: bottomLeftCorner.y },
            PointCal.multiplyVectorByScalar(leftRightDirection, subDivisor)
        );
        context.beginPath();
        context.strokeStyle = 'black';
    ...
    for (
        let i = minVerticalSmallTick;
        i <= maxVerticalSmallTick;
        i += subDivisor
    ) {
        const startPoint = PointCal.addVector(
            { x: topLeftCorner.x, y: i },
            PointCal.multiplyVectorByScalar(topDownDirection, subDivisor)
        );
        const endPoint = PointCal.addVector(
            { x: topRightCorner.x, y: i },
            PointCal.multiplyVectorByScalar(topDownDirection, subDivisor)
        );
        context.beginPath();
        context.strokeStyle = 'black';
 ...
 * @category Board
 */
export function drawRulerLegacy(
    context: CanvasRenderingContext2D,
    topLeftCorner: Point,
    topRightCorner: Point,
    bottomLeftCorner: Point,
    ...
    let maxHorizontalLargeTick =
        Math.floor(topRightCorner.x / divisor) * divisor;
    let minVerticalLargeTick = alignCoordinateSystem
        ? Math.ceil(topLeftCorner.y / divisor) * divisor
        : Math.floor(bottomLeftCorner.y / divisor) * divisor;
    let maxVerticalLargeTick = alignCoordinateSystem
        ? Math.floor(bottomLeftCorner.y / divisor) * divisor
        : Math.ceil(topLeftCorner.y / divisor) * divisor;
    let minHorizontalMediumTick =
        Math.ceil(topLeftCorner.x / halfDivisor) * halfDivisor;
    let maxHorizontalMediumTick =
        Math.floor(topRightCorner.x / halfDivisor) * halfDivisor;
    let minVerticalMediumTick = alignCoordinateSystem
        ? Math.ceil(topLeftCorner.y / halfDivisor) * halfDivisor
        : Math.floor(bottomLeftCorner.y / halfDivisor) * halfDivisor;
    let maxVerticalMediumTick = alignCoordinateSystem
        ? Math.floor(bottomLeftCorner.y / halfDivisor) * halfDivisor
        : Math.ceil(topLeftCorner.y / halfDivisor) * halfDivisor;
    let minHorizontalSmallTick =
        Math.ceil(topLeftCorner.x / subDivisor) * subDivisor;
    let maxHorizontalSmallTick =
        Math.floor(topRightCorner.x / subDivisor) * subDivisor;
    let minVerticalSmallTick = alignCoordinateSystem
        ? Math.ceil(topLeftCorner.y / subDivisor) * subDivisor
        : Math.floor(bottomLeftCorner.y / subDivisor) * subDivisor;
    let maxVerticalSmallTick = alignCoordinateSystem
        ? Math.floor(bottomLeftCorner.y / subDivisor) * subDivisor
        : Math.ceil(topLeftCorner.y / subDivisor) * subDivisor;
    ...
    const midBaseLineHeight =
        midBaseLineTextDimensions.fontBoundingBoxAscent +
        midBaseLineTextDimensions.fontBoundingBoxDescent;
    const subBaseLineTextDimensions = context.measureText(
        `${-(subDivisor + minHorizontalSmallTick)}`
    );
    const subBaseLineHeight =
        subBaseLineTextDimensions.fontBoundingBoxAscent +
        subBaseLineTextDimensions.fontBoundingBoxDescent;

    const largeHorizontalStep = Math.ceil(
        (maxHorizontalLargeTick - minHorizontalLargeTick) / divisor
    );
    for (let index = 0; index <= largeHorizontalStep; index++) {
        ...
        let resPoint = PointCal.addVector(
            { x: i, y: topLeftCorner.y },
        ...
        context.font = `bold ${20 / cameraZoomLevel}px Helvetica`;
        const textDimensions = context.measureText(
            `${Math.abs(i) % 1 == 0 ? i.toFixed(0) : i.toFixed(2)}`
        );
        const height =
            textDimensions.fontBoundingBoxAscent +
            textDimensions.fontBoundingBoxDescent;
        if (!alignCoordinateSystem) {
    ...
    const largeVerticalStep = Math.ceil(
        (maxHorizontalLargeTick - minHorizontalLargeTick) / divisor
    );
    for (let index = 0; index <= largeVerticalStep; index++) {
        ...
        resPoint = PointCal.addVector(resPoint, {
            x: textDimensions.width / 2 + textDimensions.width * 0.3,
            y: 0,
        });
        if (!alignCoordinateSystem) {
            context.fillText(
    ...
    const mediumHorizontalStep = Math.ceil(
        (maxHorizontalMediumTick - minHorizontalMediumTick) / halfDivisor
    );
    for (let index = 0; index <= mediumHorizontalStep; index++) {
        const i = minHorizontalMediumTick + index * halfDivisor;
        if (Math.floor(i * scaling) % Math.floor(divisor * scaling) == 0)
            continue;
        context.beginPath();
        context.strokeStyle = 'black';
        context.fillStyle = 'black';
    ...
    const mediumVerticalStep = Math.ceil(
        (maxVerticalMediumTick - minVerticalMediumTick) / halfDivisor
    );
    ...
    const smallHorizontalStep = Math.ceil(
        (maxHorizontalSmallTick - minHorizontalSmallTick) / subDivisor
    );
    for (let index = 0; index <= smallHorizontalStep; index++) {
        const i = minHorizontalSmallTick + index * subDivisor;
        if (
            Math.floor(i * scaling) % Math.floor(divisor * scaling) == 0 ||
            Math.floor(i * scaling) % Math.floor(halfDivisor * scaling) == 0
        )
            continue;
        context.beginPath();
        context.strokeStyle = 'black';
    ...
    const smallVerticalStep = Math.ceil(
        (maxVerticalSmallTick - minVerticalSmallTick) / subDivisor
    );
 ...
 * @category Board
 */
export function drawPositionText(
    context: CanvasRenderingContext2D,
    pos: Point,
    cameraZoomLevel: number,
    alignCoordinateSystem: boolean,
