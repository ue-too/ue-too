import { Coordinator, ComponentSchema, createGlobalComponentName, createComponentName, SerializedECSState } from "../src";
...
describe('Coordinator - State Management and Serialization', () => {
    ...
    it('should get all components for an entity', () => {
        const coordinator = new Coordinator();
        coordinator.registerComponent<{ x: number; y: number }>(GLOBAL_POSITION);
        coordinator.registerComponent<{ x: number; y: number }>(GLOBAL_VELOCITY);
        coordinator.registerComponent<{ current: number; max: number }>(GLOBAL_HEALTH);
    ...
    it('should get full state of ECS', () => {
        ...
        const entity1 = coordinator.createEntity();
        coordinator.addComponentToEntity(GLOBAL_POSITION, entity1, { x: 10, y: 20 });
        coordinator.addComponentToEntity(GLOBAL_VELOCITY, entity1, { x: 1, y: 2 });
        
        const entity2 = coordinator.createEntity();
        coordinator.addComponentToEntity(GLOBAL_POSITION, entity2, { x: 30, y: 40 });
        coordinator.addComponentToEntity(GLOBAL_HEALTH, entity2, { current: 50, max: 100 });
        ...
        expect(entity1Data!.components.get(GLOBAL_POSITION)).toEqual({ x: 10, y: 20 });
        expect(entity1Data!.components.get(GLOBAL_VELOCITY)).toEqual({ x: 1, y: 2 });
        ...
        expect(entity2Data!.components.get(GLOBAL_POSITION)).toEqual({ x: 30, y: 40 });
        expect(entity2Data!.components.get(GLOBAL_HEALTH)).toEqual({ current: 50, max: 100 });
    });
    ...
    it('should serialize ECS state to JSON-compatible format', () => {
        ...
        const entity1Serialized = serialized.entities.find(e => e.entity === entity1);
        expect(entity1Serialized).toBeDefined();
        expect(entity1Serialized!.components['Position']).toEqual({ x: 10, y: 20 });
        expect(entity1Serialized!.components['Velocity']).toEqual({ x: 1, y: 2 });
        
        const entity2Serialized = serialized.entities.find(e => e.entity === entity2);
        expect(entity2Serialized).toBeDefined();
        expect(entity2Serialized!.components['Position']).toEqual({ x: 30, y: 40 });
        expect(entity2Serialized!.components['Health']).toEqual({ current: 50, max: 100 });
    ...
    it('should throw error when serializing with non-global symbols', () => {
        ...
        const entity = coordinator.createEntity();
        coordinator.addComponentToEntity(LOCAL_COMPONENT, entity, { value: 42 });
    ...
    it('should include schemas in serialized state when available', () => {
        ...
        coordinator.addComponentToEntityWithSchema(GLOBAL_HEALTH, entity, component);
    ...
    it('should deserialize ECS state', () => {
        ...
        newCoordinator.registerComponent<{ x: number; y: number }>(GLOBAL_POSITION);
        newCoordinator.registerComponent<{ x: number; y: number }>(GLOBAL_VELOCITY);
        newCoordinator.registerComponent<{ current: number; max: number }>(GLOBAL_HEALTH);
        ...
        const pos1 = newCoordinator.getComponentFromEntity<{ x: number; y: number }>(GLOBAL_POSITION, newEntity1);
        const vel1 = newCoordinator.getComponentFromEntity<{ x: number; y: number }>(GLOBAL_VELOCITY, newEntity1);
        ...
        if (pos1 && vel1) {
            ...
            const pos2 = newCoordinator.getComponentFromEntity<{ x: number; y: number }>(GLOBAL_POSITION, newEntity2);
            const health2 = newCoordinator.getComponentFromEntity<{ current: number; max: number }>(GLOBAL_HEALTH, newEntity2);
    ...
    it('should throw error when deserializing with unregistered component', () => {
        ...
        const entity = coordinator.createEntity();
        coordinator.addComponentToEntity(GLOBAL_POSITION, entity, { x: 10, y: 20 });
    ...
    it('should handle round-trip serialization/deserialization', () => {
        ...
        coordinator1.registerComponent<{ x: number; y: number }>(GLOBAL_POSITION);
        coordinator1.registerComponent<{ x: number; y: number }>(GLOBAL_VELOCITY);
        coordinator1.registerComponent<{ current: number; max: number }>(GLOBAL_HEALTH);
        
        const entity1 = coordinator1.createEntity();
        coordinator1.addComponentToEntity(GLOBAL_POSITION, entity1, { x: 10, y: 20 });
        coordinator1.addComponentToEntity(GLOBAL_VELOCITY, entity1, { x: 1, y: 2 });
        
        const entity2 = coordinator1.createEntity();
        coordinator1.addComponentToEntity(GLOBAL_POSITION, entity2, { x: 30, y: 40 });
        coordinator1.addComponentToEntity(GLOBAL_HEALTH, entity2, { current: 50, max: 100 });
        ...
        coordinator2.registerComponent<{ x: number; y: number }>(GLOBAL_POSITION);
        coordinator2.registerComponent<{ x: number; y: number }>(GLOBAL_VELOCITY);
        coordinator2.registerComponent<{ current: number; max: number }>(GLOBAL_HEALTH);
    ...
    it('should handle deserialization with schema-based components', () => {
        ...
        const component = coordinator1.createComponentFromSchema(GLOBAL_HEALTH, { current: 75 });
        coordinator1.addComponentToEntityWithSchema(GLOBAL_HEALTH, entity, component);
        ...
        const health = coordinator2.getComponentFromEntity<Record<string, unknown>>(GLOBAL_HEALTH, allEntities[0]);
        expect(health).toEqual({ current: 75, max: 100 });
    });

    it('should handle complex nested data structures in serialization', () => {
        const coordinator = new Coordinator();
        coordinator.registerComponent<{ data: { nested: { value: number } } }>(GLOBAL_POSITION);
        ...
        const json = JSON.stringify(serialized);
        const parsed = JSON.parse(json);
        expect(parsed.entities[0].components['Position'].data.nested.value).toBe(42);
    });
});
