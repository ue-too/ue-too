...
describe('Coordinator', () => {
    ...
    it('should be able to add a component to an entity', () => {
        ...
        expect(
            coordinator.getComponentFromEntity(MOCK_COMPONENT, entity)
        ).toEqual(component);
    });
    ...
    it('should be able to set a system signature and update the system entities', () => {
        ...
        const entity = coordinator.createEntity();
        coordinator.addComponentToEntity(MOCK_COMPONENT, entity, {
            name: 'John',
            age: 30,
        });
        coordinator.setSystemSignature(MOCK_SYSTEM, 1);
    ...
    it('should be able to let system iterate over its entities based on the system signature', () => {
        ...
        coordinator.addComponentToEntity(MOCK_COMPONENT, entity3, {
            name: 'Jim',
            age: 35,
        });
    ...
    it('should only include entities with all required components in system entities', () => {
        ...
        coordinator.addComponentToEntity(Position, entityWithOnlyPosition, {
            x: 0,
            y: 0,
        });

        const entityWithOnlyVelocity = coordinator.createEntity();
        coordinator.addComponentToEntity(Velocity, entityWithOnlyVelocity, {
            x: 1,
            y: 1,
        });
        ...
        coordinator.addComponentToEntity(Position, entityWithAllComponents, {
            x: 20,
            y: 20,
        });
        coordinator.addComponentToEntity(Velocity, entityWithAllComponents, {
            x: 2,
            y: 2,
        });
        coordinator.addComponentToEntity(Health, entityWithAllComponents, {
            value: 100,
        });
        ...
        expect(movementSystem.entities.has(entityWithPositionAndVelocity)).toBe(
            true
        );
        ...
        coordinator.removeComponentFromEntity(
            Velocity,
            entityWithPositionAndVelocity
        );
        expect(movementSystem.entities.has(entityWithPositionAndVelocity)).toBe(
        ...
        coordinator.addComponentToEntity(Velocity, entityWithOnlyPosition, {
            x: 1,
            y: 1,
        });
