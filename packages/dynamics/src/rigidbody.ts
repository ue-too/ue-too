...
export abstract class BaseRigidBody implements RigidBody {
    ...
    constructor(
        center: Point,
        _orientationAngle: number = 0,
        mass: number = 50,
        isStaticBody: boolean = false,
    ...
    applyForceInOrientation(force: Point | number): void {
        ...
        if (typeof force === 'number') {
            forceTransformed = PointCal.rotatePoint(
                { x: force, y: 0 },
    ...
    step(deltaTime: number): void {
        if (this.frictionEnabled) {
            if (
                ...
                (this.linearVelocity.x == 0 &&
                    ...
                    PointCal.magnitude({ x: this.force.x, y: this.force.y }) <
                        this.staticFrictionCoeff * this.mass * 9.81)
            ) {
                ...
                let kineticFrictionDirection = PointCal.multiplyVectorByScalar(
                    PointCal.unitVector({
                ...
                let kineticFriction = PointCal.multiplyVectorByScalar(
                    kineticFrictionDirection,
                    this.dynamicFrictionCoeff * this.mass * 9.81
                );
        ...
        this.force = PointCal.addVector(this.force, {
            x: 0,
            y: 0,
            z: gravitationalForce,
        });
        const deltaLinearVelocity = PointCal.divideVectorByScalar(
            PointCal.multiplyVectorByScalar(this.force, deltaTime),
            this.mass
        );
        this._linearVelocity = PointCal.addVector(
            this._linearVelocity,
            deltaLinearVelocity
        );
        const deltaCenter = PointCal.multiplyVectorByScalar(
            this._linearVelocity,
            deltaTime
        );
...
export class Polygon extends BaseRigidBody {
    ...
    constructor(
        ...
        let numerator = this.vertices.reduce((acc, vertex, index) => {
            let nextPointIndex =
                index < this.vertices.length - 1 ? index + 1 : 0;
            let nextPoint = this.vertices[nextPointIndex];
            let crossProduct = PointCal.crossProduct(nextPoint, vertex);
            return (
                acc +
                PointCal.magnitude(crossProduct) *
                    (PointCal.dotProduct(vertex, vertex) +
                        PointCal.dotProduct(vertex, nextPoint) +
                        PointCal.dotProduct(nextPoint, nextPoint))
            );
        }, 0);
    ...
    getVerticesAbsCoord(): Point[] {
        return this.vertices.map(vertex => {
            return PointCal.addVector(
                this._center,
                PointCal.rotatePoint(vertex, this._orientationAngle)
            );
        });
    }
    ...
    getCollisionAxes(relativeBody: RigidBody): Point[] {
        return this.getVerticesAbsCoord().map((vertex, index, absVertices) => {
            let vector = PointCal.subVector(
                vertex,
                absVertices[absVertices.length - 1]
            );
            if (index > 0) {
                vector = PointCal.subVector(vertex, absVertices[index - 1]);
            }
            return PointCal.unitVector(
                PointCal.rotatePoint(vector, Math.PI / 2)
            );
        });
    }
    ...
    significantVertex(collisionNormal: Point): Point {
        let vertices = this.getVerticesAbsCoord();
        let verticesProjected = vertices.map(vertex =>
            PointCal.dotProduct(vertex, collisionNormal)
        );
        let maxIndex = verticesProjected.indexOf(
            Math.max(...verticesProjected)
        );
    ...
    getSignificantVertices(collisionNormal: Point): Point[] {
        ...
        const prevPointProjected = PointCal.dotProduct(
            prevPoint,
            collisionNormal
        );
        const nextPointProjected = PointCal.dotProduct(
            nextPoint,
            collisionNormal
        );
    ...
    getAdjacentFaces(collisionNormal: Point): {
        ...
        if (prevPointProjected > nextPointProjected) {
            ...
            adjacentFacesWithIndex.push({
                startPoint: { coord: prevPoint, index: prevPointIndex },
                endPoint: { coord: tipVertex, index: maxIndex },
            });

            // the nextface is the next face
            adjacentFacesWithIndex.unshift({
                startPoint: { coord: tipVertex, index: maxIndex },
                endPoint: { coord: nextPoint, index: nextPointIndex },
            });
            // need to get the previous face of the previous face
            let prevPrevPointIndex =
                prevPointIndex > 0 ? prevPointIndex - 1 : vertices.length - 1;
            adjacentFacesWithIndex.unshift({
                startPoint: {
                    coord: vertices[prevPrevPointIndex],
                    index: prevPrevPointIndex,
                },
                endPoint: { coord: prevPoint, index: prevPointIndex },
            });
        } else {
            ...
            let nextNextPointIndex =
                nextPointIndex < vertices.length - 1 ? nextPointIndex + 1 : 0;
            adjacentFacesWithIndex.unshift({
                startPoint: { coord: nextPoint, index: nextPointIndex },
                endPoint: {
                    coord: vertices[nextNextPointIndex],
                    index: nextNextPointIndex,
                },
            });
...
export class Circle extends BaseRigidBody {
    ...
    getMinMaxProjection(unitvector: Point): { min: number; max: number } {
        let PositiveFurthest = PointCal.addVector(
            this._center,
            PointCal.multiplyVectorByScalar(unitvector, this._radius)
        );
        let NegativeFurthest = PointCal.addVector(
            this._center,
            PointCal.multiplyVectorByScalar(unitvector, -this._radius)
        );
        return {
            min: PointCal.dotProduct(NegativeFurthest, unitvector),
            max: PointCal.dotProduct(PositiveFurthest, unitvector),
        };
    }
    ...
    getCollisionAxes(relativeBody: RigidBody): Point[] {
        return [
            PointCal.unitVector(
                PointCal.subVector(relativeBody.center, this._center)
            ),
        ];
    }
    ...
    get AABB(): { min: Point; max: Point } {
        return {
            min: PointCal.subVector(this._center, {
                x: this._radius,
                y: this._radius,
            }),
            max: PointCal.addVector(this._center, {
                x: this._radius,
                y: this._radius,
            }),
        };
    ...
    significantVertex(collisionNormal: Point): Point {
        return PointCal.addVector(
            this._center,
            PointCal.multiplyVectorByScalar(collisionNormal, this._radius)
        );
    }
