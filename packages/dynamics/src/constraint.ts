...
export class FixedPinJoint implements Constraint {
    ...
    solveWorldPinJointConstraint(dt: number) {
        ...
        const worldAnchorOnBody = PointCal.addVector(
            body.center,
            PointCal.rotatePoint(localAnchor, body.orientationAngle)
        );
...
export class PinJoint implements Constraint {
    ...
    solvePinJointConstraint(dt: number) {
        ...
        const worldAnchorA = PointCal.addVector(
            bodyA.center,
            PointCal.rotatePoint(anchorA, bodyA.orientationAngle)
        );
        const worldAnchorB = PointCal.addVector(
            bodyB.center,
            PointCal.rotatePoint(anchorB, bodyB.orientationAngle)
        );
        ...
        const relativeVelocity = PointCal.subVector(
            PointCal.addVector(
                ...
                PointCal.crossProduct(
                    { x: 0, y: 0, z: bodyB.angularVelocity },
                    rB
                )
            ),
            PointCal.addVector(
                ...
                PointCal.crossProduct(
                    { x: 0, y: 0, z: bodyA.angularVelocity },
                    rA
                )
            )
        );
        ...
        if (!bodyA.isStatic()) {
            ...
            bodyA.angularVelocity -=
                invIA * (rA.x * impulse.y - rA.y * impulse.x);
        }
        ...
        if (!bodyB.isStatic()) {
            ...
            bodyB.angularVelocity +=
                invIB * (rB.x * impulse.y - rB.y * impulse.x);
        }
    }
}
...
export function solvePinJointConstraint(
    constraint: PinJointConstraint,
    dt: number
) {
    ...
    const relativeVelocity = PointCal.subVector(
        PointCal.addVector(
            bodyB.linearVelocity,
            PointCal.crossProduct({ x: 0, y: 0, z: bodyB.angularVelocity }, rB)
        ),
        PointCal.addVector(
            bodyA.linearVelocity,
            PointCal.crossProduct({ x: 0, y: 0, z: bodyA.angularVelocity }, rA)
        )
    );
...
export function solveWorldPinJointConstraint(
    constraint: WorldPinJointConstraint,
    dt: number
) {
