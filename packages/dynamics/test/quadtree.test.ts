import { QuadTree, RectangleBound, QuadTreeObject } from "../src/quadtree";
...
describe("QuadTree", () => {
    ...
    test("Insert multiple objects within capacity", () => {
        ...
        expect(() => quadTree.retrieve(new Circle({ x: 0, y: 0 }, 5))).not.toThrow();
    });
...
describe("QuadTree Edge Cases", () => {
    ...
    test("Objects with zero size", () => {
        ...
        const genericQuadTree = new QuadTree<QuadTreeObject>(0, 
            new RectangleBound({ x: -100, y: -100 }, 200, 200)
        );
        
        genericQuadTree.insert(zeroSizeObject);
        
        expect(genericQuadTree.retrieve(zeroSizeObject)).toContain(zeroSizeObject);
    });
