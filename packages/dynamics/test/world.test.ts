import { FixedPinJoint, PinJoint } from '../src/constraint';
...
describe('World', () => {
    ...
    test('World step with collision resolution disabled', () => {
        ...
        expect(Math.abs(circle1.linearVelocity.x - initialVel1.x)).toBeLessThan(
            1
        );
        expect(Math.abs(circle2.linearVelocity.x - initialVel2.x)).toBeLessThan(
            1
        );
    });
...
describe('World Constraint System', () => {
    ...
    test('Add multiple constraints', () => {
        ...
        const constraint1 = new PinJoint(
            circle1,
            circle2,
            { x: 0, y: 0 },
            { x: 0, y: 0 }
        );
        const constraint2 = new FixedPinJoint(
            circle3,
            { x: 0, y: 0 },
            { x: 40, y: 0 }
        );
    ...
    test('World with fixed pin joint constraint', () => {
        ...
        const constraint = new FixedPinJoint(
            circle,
            { x: 0, y: 0 },
            worldAnchor
        );
