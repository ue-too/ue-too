...
import { World, QuadTree, Polygon, Circle, VisaulCircleBody, VisualPolygonBody } from "@ue-too/dynamics";
import { CameraPanEventPayload, CameraState, CameraZoomEventPayload } from "@ue-too/board";
...
for (let index = 0; index < 100; index++){
    if(index == 0){
        ...
        let body = new Polygon(getRandomPoint(0, 5000), [{x: 20, y: 10}, {x: -20, y: 10}, {x: -20, y: -10}, {x: 20, y: -10}], 0, 50);
        world.addRigidBody(index.toString(), body);
    }
}
