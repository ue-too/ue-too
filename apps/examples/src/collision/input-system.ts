import { INPUT_COMPONENT, InputComponent, PHYSICS_COMPONENT, PhysicsComponent } from "@ue-too/dynamics";
...
export class InputSystem implements System {
    ...
    constructor(coordinator: Coordinator){
        ...
        let inputComponentType = this.coordinator.getComponentType(INPUT_COMPONENT);
        if(inputComponentType === undefined){
            ...
            inputComponentType = this.coordinator.getComponentType(INPUT_COMPONENT);
        }
        this.coordinator.registerSystem("inputSystem", this);
        this.coordinator.setSystemSignature("inputSystem", 1 << physicsComponentType | 1 << inputComponentType);
    }

    update(deltaTime: number): void {
        const forceScalar = 100;
        for(const entity of this.entities){
            const inputComponent = this.coordinator.getComponentFromEntity<InputComponent>(INPUT_COMPONENT, entity);
