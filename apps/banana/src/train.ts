...
export class FormationCoordinator {
    ...
    constructor(
        formationManager: GenericEntityManager<Formation>,
        carManager: GenericEntityManager<Car>
    ) {
    ...
    removeFormation(formationNumber: number): boolean {
        ...
        if (formation.carsOrder.length > 0) {
            console.warn(
                `Formation ${formationNumber} is not empty, cannot remove`
            );
            return false;
        }
    ...
    createNewFormationWithCars(
        carNumbers: number[],
        name: string = `Formation ${carNumbers.length}`
    ): number | null {
        const cars = carNumbers.map(carNumber =>
            this._carManager.getEntity(carNumber)
        );
