...
function getPreviewCurve(
    ...
    if (
        newEndJointType.type === 'new' &&
        (newStartJointType.type === 'new' || extendAsStraightLine)
    ) {
        // a straight line

        let { flipped: tangentCalibrated, tangent: startTangent } =
            calibrateTangent(
                newStartJointType.type !== 'new'
                    ? newStartJointType.constraint.tangent
                    : PointCal.unitVectorFromA2B(
                          newStartJointType.position,
                          newEndJointType.position
                      ),
                newStartJointType.position,
                newEndJointType.position
            );
        ...
        const midPoint = {
            x:
                newStartJointType.position.x +
                (adjustedEndPosition.x - newStartJointType.position.x) / 2,
            y:
                newStartJointType.position.y +
                (adjustedEndPosition.y - newStartJointType.position.y) / 2,
        };
        ...
        let { flipped: tangentCalibrated, tangent } = calibrateTangent(
            newEndJointType.constraint.tangent,
            newEndJointType.position,
            newStartJointType.position
        );
        ...
        const previewCurveCPs = createCubicFromTangentsCurvatures(
            ...
            previewEndTangent,
            curvature,
            newEndJointType.constraint.curvature
        );
        return {
            cps: [
                previewCurveCPs.p0,
...
export function createCubicFromTangentsCurvatures(
    startPoint: Point,
    endPoint: Point,
    startTangent: Point,
    endTangent: Point,
...
function createG2Cubic(existingCurve: BCurve, targetPoint: Point) {
    ...
    const targetDistance = PointCal.distanceBetweenPoints(
        endPoint,
        targetPoint
    );
...
function createInteractiveQuadratic(
    existingCurve: BCurve,
    mousePos: Point,
    atT: number = 1
) {
