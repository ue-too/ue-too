...
export function BananaToolbar() {
  ...
  const [buildingPreset, setBuildingPreset] = useState<BuildingPreset>('medium');
  const [buildingElevation, setBuildingElevation] = useState<ELEVATION>(ELEVATION.ABOVE_1);
  ...
  useEffect(() => {
    ...
    if (selectedBuildingRef.current !== null) {
      app.buildingManager.updateBuildingHeight(selectedBuildingRef.current, buildingHeight);
    }
  }, [app, buildingHeight]);
    ...
    const unsub = app.layoutStateMachine.onStateChange((_current, next) => {
      switch (next) {
        ...
        case 'HOVER_FOR_STARTING_POINT':
          ...
          break;
        case 'IDLE':
          if (modeRef.current === 'layout' || modeRef.current === 'layout-deletion') {
            setMode('idle');
          }
          break;
      }
  ...
  const handlePointerDown = useCallback(
    (event: PointerEvent) => {
      ...
      if (currentMode === 'building-placement') {
        const existingHit = app.buildingManager.getBuildingAt(worldPosition);
        if (existingHit !== null) {
          selectedBuildingRef.current = existingHit;
          const existing = app.buildingManager.getBuilding(existingHit);
          if (existing) {
  ...
  return (
    <div className="pointer-events-auto absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Button
          variant={mode === 'layout-deletion' ? 'destructive' : 'outline'}
          size="sm"
          onClick={handleLayoutDeletionToggle}
          disabled={mode !== 'idle' && mode !== 'layout-deletion'}
        >
          {mode === 'layout-deletion' ? 'End Deletion' : 'Delete Layout'}
        </Button>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          {mode === 'train-placement' ? 'End Train Placement' : 'Place Train'}
        </Button>
        <Button variant="outline" size="sm" onClick={() => train.setThrottleStep('p5')}>
          P5
        </Button>
        <Button variant="outline" size="sm" onClick={() => train.setThrottleStep('N')}>
          Neutral
        </Button>
        <Button variant="outline" size="sm" onClick={() => train.switchDirection()}>
          Switch Dir
        </Button>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        <Button
          variant={mode === 'building-placement' ? 'default' : 'outline'}
          size="sm"
          onClick={handleBuildingPlacementToggle}
          disabled={mode !== 'idle' && mode !== 'building-placement'}
        >
          {mode === 'building-placement' ? 'End Placement' : 'Place Building'}
        </Button>
        <Button
          variant={mode === 'building-deletion' ? 'destructive' : 'outline'}
          size="sm"
          onClick={handleBuildingDeletionToggle}
          disabled={mode !== 'idle' && mode !== 'building-deletion'}
        >
          {mode === 'building-deletion' ? 'End Deletion' : 'Delete Building'}
        </Button>

        <select
          className="h-8 rounded-md border bg-background px-2 text-sm"
          value={buildingPreset}
          onChange={(e) => setBuildingPreset(e.target.value as BuildingPreset)}
        >
        ...
        <select
          ...
          onChange={(e) => setBuildingElevation(Number(e.target.value) as ELEVATION)}
        >
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Button variant="outline" size="sm" onClick={handleExportTracks}>
          Export Tracks
        </Button>
      ...
      {/* Status */}
      <p className="text-xs text-muted-foreground">Elevation: {elevation}</p>
    </div>
  );
}
