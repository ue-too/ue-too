...
export class Grid {
    ...
    private _cells: Cell[][] = [];
    private _changeObservable: Observable<[{row: number, column: number, cell: Cell}]> = new SynchronousObservable<[{row: number, column: number, cell: Cell}]>();
    private _position: Point = {x: 0, y: 0};

    constructor(private _rows: number, private _columns: number) {
        this._cells = new Array(_rows).fill(null).map(() => new Array(_columns).fill(null));
    ...
    onCellChange(callback: (row: number, column: number, cell: Cell) => void): () => void {
