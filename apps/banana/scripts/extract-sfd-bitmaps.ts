...
// ---------------------------------------------------------------------------
// 1. Collect all CJK characters from translation files
// ---------------------------------------------------------------------------
...
function collectCJKChars(): Set<string> {
    ...
    for (const file of localeFiles) {
        ...
        for (const ch of content) {
            ...
            // CJK Unified Ideographs + Extension A + common CJK ranges
            if (
                ...
                (code >= 0x2e80 && code <= 0x2eff) || // Radicals Supplement
...
function trimGrid(grid: boolean[][]): string[] {
    let firstRow = 0;
...
for (const char of chars) {
    ...
    const glyph = parseGlyph(sfd, code);
    if (!glyph) {
        console.warn(
...
console.log(
    `Extracted ${glyphs.size} glyphs (${missing} missing)`
);
