...
function step(timestamp: number){
    ...
    board.context.save();
    board.context.fillStyle = "rgba(241, 0, 0, 0.5)";
    board.context.arc(0, 0, 10, 0, 2 * Math.PI);
    board.context.fill();
    board.context.restore();
    ...
    if(capture){
        ...
        //         const imageData = board.context.getImageData(0 + col * 256, 0 + row * 256, 256, 256);
        //         downloadImageDataAsPNG(imageData, `capture_${row}_${col}.png`);
        //     }
        // }
        const imageData = board.context.getImageData(-10, -10, 20, 20);
