...
function createTestGameState(): {
    ...
    coordinator.addComponentToEntity<TurnStateComponent>(
        TURN_STATE_COMPONENT,
        player1,
        {
...
function createZone(
    ...
    coordinator.addComponentToEntity<{ cached: { entities: Entity[] } }>(
        DECK_COMPONENT,
        zone,
        {
            cached: { entities: [] },
...
function createCard(
    ...
    coordinator.addComponentToEntity<CardComponent>(
        CARD_COMPONENT,
        card,
        defaultCard
    );
...
describe('DrawCard Action', () => {
    it('should allow drawing when it is player turn and deck has cards', () => {
        const { state, player1, player1DeckZone, player1HandZone } =
            createTestGameState();
        const coordinator = state.coordinator;

        // Add a card to the deck
        createCard(coordinator, player1, player1DeckZone, {
            name: 'Test Card',
        });
    ...
    it('should not allow drawing when it is not player turn', () => {
        const { state, player1, player2, player1DeckZone } =
            createTestGameState();
        const coordinator = state.coordinator;
    ...
    it('should move card from deck to hand when executed', () => {
        ...
        // Add a card to the deck
        const card = createCard(coordinator, player1, player1DeckZone, {
            name: 'Drawn Card',
        });
        ...
        const handDeckComp = coordinator.getComponentFromEntity<{
            cached: { entities: Entity[] };
        }>(DECK_COMPONENT, player1HandZone);
        const deckDeckComp = coordinator.getComponentFromEntity<{
            cached: { entities: Entity[] };
        }>(DECK_COMPONENT, player1DeckZone);
...
describe('PlayCard Action', () => {
    ...
    it('should move card to board and deduct mana when executed', () => {
        const { state, player1, player1HandZone, player1BoardZone } =
            createTestGameState();
        const coordinator = state.coordinator;
        ...
        const boardDeckComp = coordinator.getComponentFromEntity<{
            cached: { entities: Entity[] };
        }>(DECK_COMPONENT, player1BoardZone);
...
describe('AttackCreature Action', () => {
    it('should allow attacking an enemy creature', () => {
        const { state, player1, player2, player1BoardZone, player2BoardZone } =
            createTestGameState();
        const coordinator = state.coordinator;
        ...
        const [canExecute, error] = attackCreatureAction.canExecute(
            state,
            action
        );
        expect(canExecute).toBe(true);
    ...
    it('should deal damage and tap attacker when executed', () => {
        ...
        const defenderCard = coordinator.getComponentFromEntity<CardComponent>(
            CARD_COMPONENT,
            defender
        );
        ...
        const attackerCard = coordinator.getComponentFromEntity<CardComponent>(
            CARD_COMPONENT,
            attacker
        );
...
describe('AttackPlayer Action', () => {
    it('should allow attacking an opponent player', () => {
        const { state, player1, player2, player1BoardZone } =
            createTestGameState();
        const coordinator = state.coordinator;
        ...
        const [canExecute, error] = attackPlayerAction.canExecute(
            state,
            action
        );
        expect(canExecute).toBe(true);
...
describe('EndTurn Action', () => {
    ...
    it('should untap all creatures and remove summoning sickness on new turn', () => {
        ...
        const tappedCreature = createCard(
            coordinator,
            player1,
            player1BoardZone,
