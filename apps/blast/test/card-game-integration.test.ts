...
function getCardsInZone(engine: GameEngine, playerId: Entity, zoneName: string): Entity[] {
...
function findActionByType(engine: GameEngine, actionType: string): Action | undefined {
  return engine.getValidActions().find((a) => a.type === actionType);
}

function findActionByTypeAndTarget(engine: GameEngine, actionType: string, targetId: Entity): Action | undefined {
  return engine.getValidActions().find(
    (a) => a.type === actionType && a.targetIds.length > 0 && a.targetIds[0] === targetId
  );
}
...
describe('Simple Card Game - Initialization', () => {
  ...
  it('should give each player 20 health and 1 mana', () => {
    ...
    for (const player of players) {
      const resources = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        player
      );
...
describe('Simple Card Game - Game Flow', () => {
  ...
  it('should have PlayCard actions for affordable cards in hand', () => {
    ...
    const playableCards = handCards.filter((card) => {
      const cardComp = engine.state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
      return cardComp && cardComp.cost <= 1;
    });

    // If there are affordable cards, there should be PlayCard actions
    if (playableCards.length > 0) {
      const playActions = engine.getValidActions().filter((a) => a.type === 'PlayCard');
  ...
  it('should increase mana on new turn', () => {
    ...
    // Player 1 has 1 mana
    let resources1 = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, player1);
    ...
    let resources2 = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, player2);
    ...
    resources1 = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, player1);
...
describe('Simple Card Game - Drawing Cards', () => {
  ...
  it('should draw a card from deck to hand', () => {
    const currentPlayer = engine.getCurrentPlayer()!;
    const initialHandSize = getCardsInZone(engine, currentPlayer, 'hand').length;
    const initialDeckSize = getCardsInZone(engine, currentPlayer, 'deck').length;

    const drawAction = findActionByType(engine, 'DrawCard')!;
    engine.performAction(drawAction);

    const newHandSize = getCardsInZone(engine, currentPlayer, 'hand').length;
    const newDeckSize = getCardsInZone(engine, currentPlayer, 'deck').length;
...
describe('Simple Card Game - Playing Cards', () => {
  ...
  it('should play a card from hand to board', () => {
    ...
    if (affordableCard) {
      const playAction = findActionByTypeAndTarget(engine, 'PlayCard', affordableCard)!;
  ...
  it('should deduct mana when playing a card', () => {
    ...
    if (affordableCard) {
      const initialMana = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        currentPlayer
      )?.mana;
      ...
      const newMana = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        currentPlayer
      )?.mana;
  ...
  it('should give creatures summoning sickness when played', () => {
    ...
    const affordableCreature = handCards.find((card) => {
      const cardComp = engine.state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
      return cardComp && cardComp.cost <= 1 && cardComp.cardType === 'Creature';
    });

    if (affordableCreature) {
      const playAction = findActionByTypeAndTarget(engine, 'PlayCard', affordableCreature)!;
      engine.performAction(playAction);
...
describe('Simple Card Game - Combat', () => {
  ...
  it('should not allow attacking with creatures that have summoning sickness', () => {
    ...
    if (affordableCreature) {
      ...
      const attackActions = engine.getValidActions().filter(
        (a) =>
  ...
  it('should allow attacking after summoning sickness wears off', () => {
    ...
    if (affordableCreature) {
      ...
      const attackPlayerActions = engine.getValidActions().filter(
        (a) => a.type === 'AttackPlayer' && a.targetIds[0] === affordableCreature
      );
  ...
  it('should deal damage to opponent when attacking player', () => {
    ...
    const resources1 = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(
      RESOURCE_COMPONENT,
      player1
    )!;
    resources1.mana = 5;
    ...
    if (creature) {
      // Play the creature
      const playAction = findActionByTypeAndTarget(engine, 'PlayCard', creature)!;
      engine.performAction(playAction);
      ...
      const creatureCard = engine.state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, creature)!;
      const power = creatureCard.power ?? 0;

      // Get initial health
      const player2Resources = engine.state.coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        player2
      )!;
      ...
      // Attack player 2
      const attackAction = engine.getValidActions().find(
        (a) => a.type === 'AttackPlayer' && a.targetIds[0] === creature && a.targetIds[1] === player2
      )!;
...
describe('Simple Card Game - Valid Actions', () => {
  ...
  it('should not include PlayCard for cards that cost more than available mana', () => {
    ...
    for (const action of playActions) {
      const cardComp = engine.state.coordinator.getComponentFromEntity<CardComponent>(
        CARD_COMPONENT,
        action.targetIds[0]
      )!;
