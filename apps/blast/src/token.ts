...
export const DECK_COMPONENT: ComponentName =
    createGlobalComponentName('DeckComponent');
...
export class StackableTokenSystem implements System {
    ...
    constructor(coordinator: Coordinator) {
        ...
        let locationComponentType =
            this._coordinator.getComponentType(LOCATION_COMPONENT);
        if (locationComponentType == undefined) {
            this._coordinator.registerComponent<LocationComponent>(
                LOCATION_COMPONENT
            );
        ...
        let deckComponentType =
            this._coordinator.getComponentType(DECK_COMPONENT);
        ...
        this._coordinator.setSystemSignature(
            DECK_SYSTEM,
            (1 << locationComponentType) | (1 << deckComponentType)
        );
    }
    ...
    shuffle(at: number): void {
        ...
        for (let i = 0; i < shuffledEntities.length; i++) {
            ...
            const location =
                this._coordinator.getComponentFromEntity<LocationComponent>(
                    LOCATION_COMPONENT,
                    entity
                );
            if (!location) {
                throw new Error(
                    'Location component not found for entity ' + entity
                );
            }
    ...
    _cacheDeckOrder(atDeck: number, entities: number[]): void {
        const deck = this._coordinator.getComponentFromEntity<DeckComponent>(
            DECK_COMPONENT,
            atDeck
        );
        if (!deck) {
    ...
    _getSortedEntitiesWithLocation(
        atDeck: number
    ): { entity: number; location: LocationComponent }[] {
    ...
    peek(atLocation: number, size: number): number | undefined {
        ...
        if (!deck) {
            throw new Error(
                atLocation + ' is not a deck, thus cannot be peeked'
            );
        }
        return deck.cached.entities.length > 0
            ? deck.cached.entities[0]
            : undefined;
    }
    ...
    addToBottomOf(deck: number, entity: Entity): void {
        ...
        if (!entityLocation) {
            throw new Error(
                entity +
                    ' can not be in a location, thus cannot be added to ' +
                    deck
            );
        }
        const currentCachedEntitiesAtDeck =
            this._coordinator.getComponentFromEntity<DeckComponent>(
                DECK_COMPONENT,
                deck
            );
