...
export const HEALTH_COMPONENT: ComponentName =
    createGlobalComponentName('HealthComponent');

export function subtractHealth(
    entity: Entity,
    amount: number,
    coordinator: Coordinator
): void {
    const healthComponent = coordinator.getComponentFromEntity<HealthComponent>(
        HEALTH_COMPONENT,
        entity
    );
    if (!healthComponent) {
...
export function registerNumberComponent(
