...
export const OWNERSHIP_COMPONENT: ComponentName =
    createGlobalComponentName('OwnershipComponent');
export const OWNERSHIP_SYSTEM: SystemName =
    createGlobalSystemName('OwnershipSystem');
export function setOwner(
    entity: Entity,
    owner: Entity,
    coordinator: Coordinator
): void {
    const ownershipComponent =
...
export function removeFromOwner(
    entity: Entity,
    owner: Entity,
    coordinator: Coordinator
): void {
...
export class OwnershipSystem implements System {
    ...
    constructor(coordinator: Coordinator) {
        ...
        let ownershipComponentType =
            this.coordinator.getComponentType(OWNERSHIP_COMPONENT);
        if (ownershipComponentType == undefined) {
            this.coordinator.registerComponent<OwnershipComponent>(
                OWNERSHIP_COMPONENT
            );
            ownershipComponentType =
                this.coordinator.getComponentType(OWNERSHIP_COMPONENT)!;
        }
        if (ownershipComponentType == undefined) {
        ...
        this.coordinator.setSystemSignature(
            OWNERSHIP_SYSTEM,
            1 << ownershipComponentType
        );
    }

    getOwnerShipComponentOf(entity: Entity): OwnershipComponent | null {
        const ownershipComponent =
            this.coordinator.getComponentFromEntity<OwnershipComponent>(
                OWNERSHIP_COMPONENT,
                entity
            );
        if (!ownershipComponent) {
    ...
    getOwnerEntities(owner: Entity): Entity[] {
        return Array.from(this.entities).filter(
            entity => this.getOwnerOf(entity) === owner
        );
    }

    countEntitiesWithComponentByOwner(
        ...
        for (const entity of entities) {
            const component = this.coordinator.getComponentFromEntity<unknown>(
                componentName,
                entity
            );
            if (
