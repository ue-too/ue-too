...
export const GamePreview: React.FC<GamePreviewProps> = ({ gameDefinition }) => {
  ...
  const createEngine = useCallback(() => {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : 'Unknown error loading game');
      setEngine(null);
    }
  }, [gameDefinition]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: 'bold', fontSize: '16px' }}>Cannot load game definition</span>
        </div>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
            {validationResult.errors.map((error, idx) => (
              <li key={idx} style={{ color: '#856404', marginBottom: '4px' }}>
                {error.message}
                {error.path.length > 0 && (
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: 'bold', fontSize: '16px' }}>Failed to load game</span>
        </div>

        <p style={{ color: '#c62828', marginBottom: '16px' }}>{loadError}</p>
