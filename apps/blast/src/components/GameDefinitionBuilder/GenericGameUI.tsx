 ...
 */
import type { Coordinator, Entity } from '@ue-too/ecs';
...
function getAllPlayers(coordinator: Coordinator): PlayerInfo[] {
    ...
    for (const entity of coordinator.getAllEntities()) {
        ...
        if (playerComp) {
            ...
            const resourceComp = coordinator.getComponentFromEntity(
                resourceName,
                entity
            );
            if (resourceComp) {
...
function groupActionsByType(
    ...
    const grouped = new Map<
        string,
        Array<{ type: string; targetIds: Entity[] }>
    >();
...
export const GenericGameUI: React.FC<GenericGameUIProps> = ({
    ...
    const winner = winnerEntity
        ? players.find(p => p.entity === winnerEntity)
        : null;
            ...
            const matchingAction = validActions.find(
                a =>
                    ...
                    JSON.stringify(a.targetIds) ===
                        JSON.stringify(action.targetIds)
            );
        ...
        if (newTargets.length >= targetingMode.requiredTargetCount) {
            ...
            const matchingAction = validActions.find(
                a =>
                    ...
                    JSON.stringify(a.targetIds) ===
                        JSON.stringify(testAction.targetIds)
            );
    ...
    const getValidTargetsForCurrentSlot = (): Entity[] => {
        ...
        const matchingActions = validActions.filter(
            a =>
                a.type === targetingMode.actionType &&
                a.targetIds.length >= targetingMode.collectedTargets.length &&
                targetingMode.collectedTargets.every(
                    (target, idx) => a.targetIds[idx] === target
                )
        );
    ...
    const renderPlayerStats = (player: PlayerInfo) => {
        ...
        const isWinner =
            isGameOver && winner && winner.entity === player.entity;
        ...
        return (
            ...
            >
                ...
                <div style={{ display: 'flex', gap: '16px', fontSize: '13px' }}>
                    ...
                    {typeof props.maxMana === 'number' &&
                        props.maxMana !== props.mana && (
                ...
                >
                    {isPrivate ? (
                        ...
                        zone.entities.map(entityId => {
                            const isValidTarget = targetingMode
                                ? isValidTargetForCurrentSlot(entityId)
                                : false;
                            const isHighlighted =
                                targetingMode !== null && isValidTarget;
                            ...
                            return (
                                <EntityDisplay
                                    ...
                                    isSelected={
                                        ...
                                        (targetingMode !== null &&
                                            targetingMode.collectedTargets.includes(
                                                entityId
                                            ))
                                    }
                                    onClick={() => {
                                        if (targetingMode) {
                                            if (isValidTarget) {
                                                handleEntityClickInTargetingMode(
                                                    entityId
                                                );
                                            }
                                        } else {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {(zonesByOwner.get(null) ?? []).length > 0 && (
                ...
                >
                    ...
                    {(zonesByOwner.get(null) ?? []).map(zone =>
                        renderZone(zone, 'Shared')
                    )}
                </div>
            )}
            ...
            >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {Array.from(actionsByType.entries()).map(
                        ([actionType, actions]) => {
                            // For actions with no targets or single target type, show one button
                            const hasTarget = actions.some(
                                a => a.targetIds.length > 0
                            );
                            const canExecute =
                                !hasTarget ||
                                (selectedEntity &&
                                    actions.some(a =>
                                        a.targetIds.includes(selectedEntity)
                                    ));
                            ...
                            const actionDef = gameDefinition.actions?.find(
                                a => a.name === actionType
                            );
                            ...
                            const canStartMultiTarget =
                                needsMultipleTargets &&
                                canUseSelectedEntityAsFirstTarget;
                            ...
                            return (
                                <button
                                    ...
                                    style={{
                                        padding: '8px 16px',
                                        backgroundColor: isTargetingThisAction
                                            ? '#ff9800'
                                            : isGameOver ||
                                                !canExecute ||
                                                (needsMultipleTargets &&
                                ...
                                >
                                    {actionType}
                                    {isTargetingThisAction &&
                                        ` (${targetingMode.collectedTargets.length}/${targetingMode.requiredTargetCount})`}
                                    {hasTarget &&
                                        selectedEntity &&
                                        canExecute &&
                                        !needsMultipleTargets &&
                ...
                {targetingMode && (
                    ...
                    >
                        ...
                        >
                            Target {targetingMode.collectedTargets.length + 1}{' '}
                            of {targetingMode.requiredTargetCount}
                            {targetingMode.collectedTargets.length > 0 && (
                                <span style={{ marginLeft: '8px' }}>
                                    Selected:{' '}
                                    {targetingMode.collectedTargets.map(
                                        (id, idx) => (
                                            <span
                                                key={id}
            ...
            >
                ...
                {showDebug && (
                    <div style={{ marginTop: '8px', fontSize: '12px' }}>
                        ...
                        <div>
                            Total entities:{' '}
                            {coordinator.getAllEntities().length}
                        </div>
