...
function getEntityDisplayName(
    ...
    const markerComponent = coordinator.getComponentFromEntity(
        markerComponentName,
        entityId
    );
    if (markerComponent && typeof markerComponent === 'object') {
        ...
        if (
            ...
            const playerLabel =
                typeof playerIndex === 'number' ? ` (P${playerIndex + 1})` : '';
    ...
    for (const [compName] of Object.entries(componentSchemas)) {
        ...
        if (component && typeof component === 'object') {
            for (const prop of nameProperties) {
                if (
                    prop in component &&
                    typeof (component as Record<string, unknown>)[prop] ===
                        'string'
                ) {
...
function getEntityState(
    ...
    for (const [compName] of Object.entries(componentSchemas)) {
        ...
        if (component && typeof component === 'object') {
            ...
            if ('summoningSickness' in comp)
                result.sick = Boolean(comp.summoningSickness);
        }
    }
    ...
    for (const [compName, compDef] of Object.entries(componentSchemas)) {
        ...
        if (component && typeof component === 'object') {
            ...
            for (const [propName, propDef] of Object.entries(
                ...
                if (
                    ['name', 'cardName', 'displayName', 'title'].includes(
                        propName
                    )
                )
                    continue;
                ...
                if (
                    ['tapped', 'summoningSickness', 'attacksThisTurn'].includes(
                        propName
                    )
                )
                    continue;
            ...
            <div style={{ fontSize: '10px' }}>
                {properties.slice(0, 4).map((prop, idx) => (
                    ...
                    >
                        ...
                        <span
                            style={{
                                color: getPropertyColor(prop.label),
                                fontWeight: 500,
                            }}
                        >
                            {String(prop.value)}
