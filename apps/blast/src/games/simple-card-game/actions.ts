...
import { CustomEffect, EmitEvent, CompositeEffect } from '../../board-game-engine/action-system/effects';
import {
  IsPlayerTurn,
  PhaseCheck,
  CustomPrecondition,
} from '../../board-game-engine/action-system/preconditions';
import type { GameState, ActionContext } from '../../board-game-engine/core/types';
import {
  CARD_COMPONENT,
  RESOURCE_COMPONENT,
...
function getPlayerDeckZone(ctx: StateContext, playerId: Entity): Entity | null {
  ...
  for (const entity of allEntities) {
    const zoneComp = ctx.state.coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, entity);
    if (zoneComp && zoneComp.name === 'deck' && zoneComp.owner === playerId) {
      return entity;
    }
  }
  return null;
...
function getPlayerHandZone(ctx: StateContext, playerId: Entity): Entity | null {
  ...
  for (const entity of allEntities) {
    const zoneComp = ctx.state.coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, entity);
    if (zoneComp && zoneComp.name === 'hand' && zoneComp.owner === playerId) {
      return entity;
    }
  }
  return null;
 ...
 */
function getPlayerBoardZone(ctx: StateContext, playerId: Entity): Entity | null {
  const allEntities = ctx.state.coordinator.getAllEntities();
  for (const entity of allEntities) {
    const zoneComp = ctx.state.coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, entity);
    if (zoneComp && zoneComp.name === 'board' && zoneComp.owner === playerId) {
      return entity;
    }
  }
  return null;
 ...
 */
function getCardsInZone(ctx: StateContext, zoneName: string, playerId: Entity): Entity[] {
...
function getAttackableCreatures(ctx: StateContext, playerId: Entity): Entity[] {
  ...
  return boardCards.filter((card) => {
    const cardComp = coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
...
export const drawCardAction = new ActionDefinition({
  ...
  preconditions: [
    ...
    new CustomPrecondition(
      (ctx) => {
        const turnState = ctx.state.coordinator.getComponentFromEntity<TurnStateComponent>(
          TURN_STATE_COMPONENT,
          ctx.actor
        );
    ...
    new CustomPrecondition(
      (ctx) => {
        ...
        const deckComp = ctx.state.coordinator.getComponentFromEntity<{ cached: { entities: Entity[] } }>(
          DECK_COMPONENT,
          deckZone
        );
...
export const playCardAction = new ActionDefinition({
  ...
  preconditions: [
    ...
    new CustomPrecondition(
      (ctx) => {
        if (ctx.targets.length === 0) return false;
        return isInZone(ctx.state.coordinator, ctx.targets[0], 'hand', ctx.actor);
      },
    ...
    new CustomPrecondition(
      (ctx) => {
        if (ctx.targets.length === 0) return false;
        const cardComp = ctx.state.coordinator.getComponentFromEntity<CardComponent>(
          CARD_COMPONENT,
          ctx.targets[0]
        );
  ...
  costs: [
    ...
    new CustomEffect((ctx) => {
      const coordinator = ctx.state.coordinator;
      const cardComp = coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, ctx.targets[0]);
  ...
  effects: [
    new CustomEffect((ctx) => {
      ...
      const locationComp = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, card);
      if (locationComp) {
      ...
      const cardComp = coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
      if (cardComp && cardComp.cardType === 'Creature') {
        const existingState = coordinator.getComponentFromEntity<CardStateComponent>(
          CARD_STATE_COMPONENT,
          card
        );
        if (!existingState) {
          coordinator.addComponentToEntity<CardStateComponent>(CARD_STATE_COMPONENT, card, {
            tapped: false,
  ...
  targetSelector: (state, actor) => {
    ...
    const resources = coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, actor);
...
export const attackCreatureAction = new ActionDefinition({
  ...
  preconditions: [
    ...
    new CustomPrecondition(
      (ctx) => {
        ...
        return isInZone(ctx.state.coordinator, attacker, 'board', ctx.actor);
      },
    ...
    new CustomPrecondition(
      (ctx) => {
        const cardState = ctx.state.coordinator.getComponentFromEntity<CardStateComponent>(
          CARD_STATE_COMPONENT,
          ctx.targets[0]
        );
        if (!cardState) return false;
  ...
  effects: [
    new CustomEffect((ctx) => {
      ...
      const attackerState = coordinator.getComponentFromEntity<CardStateComponent>(
        CARD_STATE_COMPONENT,
        attacker
      );
      ...
      if (defenderCard.toughness !== undefined && attackerCard.power !== undefined) {
        ...
        if (defenderCard.toughness <= 0) {
          ...
          if (owner) {
            ...
            const allEntities = coordinator.getAllEntities();
            for (const entity of allEntities) {
              const zoneComp = coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, entity);
              if (zoneComp && zoneComp.name === 'discard' && zoneComp.owner === owner) {
                const locationComp = coordinator.getComponentFromEntity<LocationComponent>(
                  LOCATION_COMPONENT,
                  defender
                );
                if (locationComp) {
                  // Remove from board
                  const boardZone = getPlayerBoardZone({ state: ctx.state }, owner);
                  if (boardZone) {
                    const boardDeckComp = coordinator.getComponentFromEntity<{
                      cached: { entities: Entity[] };
                    }>(DECK_COMPONENT, boardZone);
                    if (boardDeckComp) {
                      const idx = boardDeckComp.cached.entities.indexOf(defender);
                      if (idx !== -1) boardDeckComp.cached.entities.splice(idx, 1);
                    }
                  }

                  // Add to discard
                  locationComp.location = entity;
                  const discardDeckComp = coordinator.getComponentFromEntity<{
                    cached: { entities: Entity[] };
                  }>(DECK_COMPONENT, entity);
                  if (discardDeckComp) {
                    discardDeckComp.cached.entities.push(defender);
                  }
                }
                break;
              }
      ...
      if (attackerCard.toughness !== undefined && defenderCard.power !== undefined) {
              ...
              if (zoneComp && zoneComp.name === 'discard' && zoneComp.owner === owner) {
                ...
                if (locationComp) {
                  ...
                  if (boardZone) {
                    ...
                    if (boardDeckComp) {
                      const idx = boardDeckComp.cached.entities.indexOf(attacker);
                      if (idx !== -1) boardDeckComp.cached.entities.splice(idx, 1);
                    }
                  }
                  ...
                  if (discardDeckComp) {
                    discardDeckComp.cached.entities.push(attacker);
                  }
                }
                break;
              }
    ...
    new EmitEvent('CreatureAttacked', (ctx) => {
      const attackerCard = ctx.state.coordinator.getComponentFromEntity<CardComponent>(
        CARD_COMPONENT,
        ctx.targets[0]
      );
      const defenderCard = ctx.state.coordinator.getComponentFromEntity<CardComponent>(
        CARD_COMPONENT,
        ctx.targets[1]
      );
      return {
...
export const attackPlayerAction = new ActionDefinition({
  ...
  effects: [
    new CustomEffect((ctx) => {
      ...
      const targetResources = coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        targetPlayer
      );
...
export const activateAbilityAction = new ActionDefinition({
  ...
  preconditions: [
    ...
    new CustomPrecondition(
      (ctx) => {
        if (ctx.targets.length === 0) return false;
        return isInZone(ctx.state.coordinator, ctx.targets[0], 'board', ctx.actor);
      },
  ...
  effects: [
    new CustomEffect((ctx) => {
      ...
      let cardState = coordinator.getComponentFromEntity<CardStateComponent>(CARD_STATE_COMPONENT, card);
      if (!cardState) {
        coordinator.addComponentToEntity<CardStateComponent>(CARD_STATE_COMPONENT, card, {
          tapped: true,
  ...
  targetSelector: (state, actor) => {
    ...
    for (const card of boardCards) {
      const cardComp = state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
      const cardState = state.coordinator.getComponentFromEntity<CardStateComponent>(
        CARD_STATE_COMPONENT,
        card
      );
...
export const endTurnAction = new ActionDefinition({
  ...
  effects: [
    new CustomEffect((ctx) => {
      ...
      const newPlayerBoardCards = getCardsInZone(ctx, 'board', players[nextIndex]);
      ...
      const newPlayerResources = coordinator.getComponentFromEntity<ResourceComponent>(
        RESOURCE_COMPONENT,
        players[nextIndex]
      );
      if (newPlayerResources && newPlayerResources.mana < newPlayerResources.maxMana) {
        newPlayerResources.mana = Math.min(newPlayerResources.mana + 1, newPlayerResources.maxMana);
      }

      // Reset hasDrawnThisTurn for the new active player
      const newPlayerTurnState = coordinator.getComponentFromEntity<TurnStateComponent>(
        TURN_STATE_COMPONENT,
        players[nextIndex]
      );
