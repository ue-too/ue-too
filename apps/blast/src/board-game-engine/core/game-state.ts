...
import type { Event, GameMetadata } from './types';
...
export class GameState {
    ...
    constructor(
        ...
        initializeGameManager: boolean = true
    ) {
    ...
    private initializeGameManager(): Entity {
        ...
        if (
            this.coordinator.getComponentType(GAME_MANAGER_COMPONENT) === null
        ) {
            this.coordinator.registerComponent<GameManagerComponent>(
    ...
    private findGameManagerEntity(): Entity {
        ...
        for (const entity of allEntities) {
            const component =
                this.coordinator.getComponentFromEntity<GameManagerComponent>(
                    GAME_MANAGER_COMPONENT,
                    entity
                );
            if (component) {
    ...
    getAllPlayers(): Entity[] {
        ...
        for (const entity of allEntities) {
            const playerComp =
                this.coordinator.getComponentFromEntity<PlayerComponent>(
                    PLAYER_COMPONENT,
                    entity
                );
            if (playerComp) {
    ...
    isPlayer(entity: Entity): boolean {
        return (
            this.coordinator.getComponentFromEntity(
                PLAYER_COMPONENT,
                entity
            ) !== null
        );
