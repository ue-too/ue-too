...
export class ResourceCheck extends BasePrecondition {
    ...
    getErrorMessage(context: ActionContext): string {
        ...
        const component = entity
            ? context.state.coordinator.getComponentFromEntity<
                  Record<string, any>
              >(this.config.componentName, entity)
            : null;
        const currentValue = component
            ? component[this.config.property]
            : 'N/A';
        const targetValue = this.resolveNumber(this.config.value, context);
...
export class ZoneHasEntities extends BasePrecondition {
    ...
    check(context: ActionContext): boolean {
        ...
        // Check min
        if (
            this.config.minCount !== undefined &&
        ...
        // Check max
        if (
            this.config.maxCount !== undefined &&
...
export class EntityInZone extends BasePrecondition {
    ...
    check(context: ActionContext): boolean {
        ...
        const locationComp = context.state.coordinator.getComponentFromEntity<{
            location: Entity;
        ...
        const zones = Array.isArray(this.config.zone)
            ? this.config.zone
            : [this.config.zone];
...
export class OwnerCheck extends BasePrecondition {
    ...
    check(context: ActionContext): boolean {
        ...
        const expectedOwner = this.resolveEntity(
            this.config.expectedOwner,
            context
        );
        if (expectedOwner === null) return false;

        const ownerComp = context.state.coordinator.getComponentFromEntity<
            Record<string, any>
        >(this.config.ownerComponent, entity);
