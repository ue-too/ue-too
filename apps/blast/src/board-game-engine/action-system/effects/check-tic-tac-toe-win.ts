...
export class CheckTicTacToeWin extends ECSEffect {
  apply(context: ActionContext): void {
    ...
    const markers: Array<{ entity: Entity; marker: MarkerComponent; location: GridLocationComponent }> = [];

    for (const entity of allEntities) {
      const marker = coordinator.getComponentFromEntity<MarkerComponent>(MARKER_COMPONENT, entity);
      const location = coordinator.getComponentFromEntity<GridLocationComponent>(GRID_LOCATION_COMPONENT, entity);
      
      if (marker && location && location.row !== null && location.column !== null) {
    ...
    for (const player of players) {
      const playerComp = coordinator.getComponentFromEntity<PlayerComponent>(PLAYER_COMPONENT, player);
      ...
      const playerNumber = playerComp.playerNumber ?? (players.indexOf(player) + 1);
      const playerIndex = playerNumber - 1; // Convert to 0-based index
      const playerMarkers = markers.filter(m => m.marker.playerIndex === playerIndex);
      ...
      if (this.hasWon(playerMarkers)) {
        ...
        coordinator.addComponentToEntity<PlayerComponent>(PLAYER_COMPONENT, player, updatedPlayerComp);
      }
    }
  }
   ...
   */
  private hasWon(markers: Array<{ location: GridLocationComponent }>): boolean {
    ...
    for (const marker of markers) {
      ...
      if (row !== null && col !== null && row >= 0 && row < 3 && col >= 0 && col < 3) {
        grid[row][col] = true;
      }
    }
