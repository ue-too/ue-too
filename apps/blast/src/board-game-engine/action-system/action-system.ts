...
export class ActionSystem {
  ...
  executeAction(state: GameState, action: Action): [GameState, Event[]] {
    ...
    const definition = this.getDefinition(action.type);
    if (!definition) {
      throw new ActionValidationError(action, null, `Unknown action type: ${action.type}`);
    }

    // Validate
    const [isValid, errorMessage] = definition.canExecute(state, action);
    if (!isValid) {
      throw new ActionValidationError(action, null, errorMessage ?? 'Action validation failed');
    }
