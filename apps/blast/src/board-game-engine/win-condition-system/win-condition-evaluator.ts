...
import type { WinConditionSchema, ResolverContext, ConditionDefinition } from '../schema/types';
...
export class WinConditionEvaluator {
  ...
  evaluateWinConditions(
    ...
    for (const winCondition of winConditions) {
      ...
      for (const player of players) {
        ...
        if (this.evaluateCondition(winCondition.condition, state, resolverContext)) {
          // Condition matched - determine winner
          const winner = this.resolveWinner(winCondition, state, resolverContext);
          const endReason = winCondition.name || winCondition.description || `Win condition ${winCondition.id} met`;
  ...
  private resolveWinner(
    ...
    if (winCondition.loser) {
      const loser = this.resolver.resolveEntity(winCondition.loser, context);
      if (loser !== null) {
