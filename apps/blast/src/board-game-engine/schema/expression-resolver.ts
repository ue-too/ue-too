...
import { type Entity, type Coordinator, type ComponentName, createGlobalComponentName } from '@ue-too/ecs';
import type {
  ResolverContext,
  ConditionDefinition,
  NumberExpression,
...
import { ZONE_COMPONENT, DECK_COMPONENT } from '../core/game-state';
...
export class ExpressionResolver {
   ...
   */
  resolveEntity(expr: EntityExpression | Entity, context: ResolverContext): Entity | null {
    ...
    switch (parts[0]) {
      ...
      case 'param':
        ...
        return typeof paramValue === 'number' ? (paramValue as Entity) : null;
      ...
      case 'context':
        ...
        return typeof ctxValue === 'number' ? (ctxValue as Entity) : null;

      case 'event':
        // Get from event data
        if (parts.length < 2 || !context.event) return null;
        const eventValue = context.event.data[parts[1]];
        return typeof eventValue === 'number' ? (eventValue as Entity) : null;
      ...
      case 'component':
        ...
        return this.resolveComponentValue(parts.slice(1), context) as Entity | null;
  ...
  resolveZone(expr: ZoneExpression | Entity, context: ResolverContext): Entity | null {
    ...
    if (ownerPart === 'actor') {
      ...
      owner = typeof paramValue === 'number' ? (paramValue as Entity) : null;
  ...
  resolveNumber(expr: NumberExpression, context: ResolverContext): number {
    ...
    if (expr.startsWith('$add(')) {
      ...
      return args.reduce((sum, arg) => sum + this.resolveNumber(arg.trim(), context), 0);
    }

    if (expr.startsWith('$multiply(')) {
      const args = this.extractFunctionArgs(expr, '$multiply(');
      return args.reduce((product, arg) => product * this.resolveNumber(arg.trim(), context), 1);
    }
    ...
    switch (parts[0]) {
      ...
      case 'component':
        const value = this.resolveComponentValue(parts.slice(1), context);
  ...
  resolveValue<T>(expr: ValueExpression<T>, context: ResolverContext): T {
    ...
    switch (parts[0]) {
      ...
      case 'event':
        if (parts.length < 2 || !context.event) return expr as unknown as T;
        return context.event.data[parts[1]] as T;

      case 'context':
        if (parts.length < 2 || !context.effectContext) return expr as unknown as T;
   ...
   */
  evaluateCondition(condition: ConditionDefinition, context: ResolverContext): boolean {
    switch (condition.type) {
      case 'isPlayerTurn':
        return context.actor !== null && context.actor === context.state.activePlayer;
  ...
  private resolveComponentValue(parts: string[], context: ResolverContext): unknown {
    ...
    try {
      const component = coordinator.getComponentFromEntity(globalComponentName, entity);
  ...
  private resolveCount(zoneExpr: string, context: ResolverContext): number {
    ...
    const coordinator = context.state.coordinator as Coordinator;
    try {
      const deckComponent = coordinator.getComponentFromEntity(DECK_COMPONENT, zone) as
  ...
  private evaluateResourceCheck(
    ...
    try {
      const component = coordinator.getComponentFromEntity(globalComponentName, entity) as Record<
        string,
        unknown
      > | null;
  ...
  private evaluateEntityInZone(
    ...
    try {
      const location = coordinator.getComponentFromEntity(locationComponentName, entity) as {
        location: Entity;
      } | null;
      if (!location) return false;

      const zones = Array.isArray(condition.zone) ? condition.zone : [condition.zone];
  ...
  private evaluateComponentValueCheck(
    condition: Extract<ConditionDefinition, { type: 'componentValueCheck' }>,
    context: ResolverContext
  ): boolean {
    const entity = this.resolveEntity(condition.entity, context);
    if (entity === null) return false;
  ...
  private evaluateOwnerCheck(
    ...
    const entity = this.resolveEntity(condition.entity, context);
    const expectedOwner = this.resolveEntity(condition.expectedOwner, context);
    ...
    try {
      const ownerComp = coordinator.getComponentFromEntity(ownerComponentName, entity) as {
        owner: Entity;
      } | null;
  ...
  private evaluateEntityExists(
    ...
    if (condition.requiredComponent) {
      ...
      const globalComponentName =
        this.getComponentName(condition.requiredComponent);
    ...
    try {
      ...
      if (condition.filter) {
        entities = entities.filter((entity) => {
          ...
          return this.evaluateCondition(condition.filter!, filterContext);
        });
      }

      const count = entities.length;

      if (condition.minCount !== undefined && count < condition.minCount) {
        return false;
      }

      if (condition.maxCount !== undefined && count > condition.maxCount) {
        return false;
      }
