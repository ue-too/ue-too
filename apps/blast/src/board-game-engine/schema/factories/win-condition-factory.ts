...
export class WinConditionFactory {
    ...
    createWinCondition(schema: WinConditionSchema): WinCondition {
        ...
        if (schema.winner) {
            this.validateEntityExpression(
                schema.winner,
                `win condition ${schema.id} winner`
            );
        }
     ...
     */
    private validateEntityExpression(
        expr: string | number,
        context: string
    ): void {
        ...
        if (typeof expr === 'string') {
            ...
            if (!expr.startsWith('$')) {
                throw new Error(
                    `Invalid entity expression in ${context}: ${expr}. Must start with $ or be a number.`
                );
            }
