...
export class ActionFactory {
    ...
    createAction(definition: ActionDefinitionSchema): ActionDefinition {
        const preconditions = definition.preconditions
            ? this.preconditionFactory.createPreconditions(
    ...
    private createTargetSelector(
        ...
        if (targeting.types && targeting.types.length > 0) {
            return (state, actor) => {
                ...
                if (
                    typeof targeting.count === 'number' &&
                    targeting.count === 1
                ) {
                    const validEntities = this.findValidTargets(
                        state,
                        actor,
                        targeting.types![0].query
                    );
                ...
                if (typeof targeting.count === 'number') {
                    // Exact count
                    return this.generateCombinationsOfSize(
                        validEntities,
                        targeting.count
                    );
                }
                ...
                for (let size = min; size <= max; size++) {
                    results.push(
                        ...this.generateCombinationsOfSize(validEntities, size)
                    );
                }
        ...
        if (query.hasComponent && query.hasComponent.length > 0) {
            candidateEntities = candidateEntities.filter(entity => {
                return query.hasComponent!.every(compName => {
                    ...
                    try {
                        const comp = coordinator.getComponentFromEntity(
                            globalName,
                            entity
                        );
