...
export class EffectFactory {
  ...
  private createMoveEntityEffect(def: MoveEntityEffectDefinition): Effect {
    ...
    return new MoveEntity({
      ...
      fromZone: def.fromZone
        ? (ctx: ActionContext) => this.resolveZoneFromContext(def.fromZone!, ctx)
        : undefined,
      toZone: (ctx: ActionContext) => this.resolveZoneFromContext(def.toZone, ctx),
      locationComponent,
      zoneListComponent,
  ...
  private createModifyResourceEffect(def: ModifyResourceEffectDefinition): Effect {
    ...
    return new ModifyResource({
      ...
      amount: (ctx: ActionContext) => this.resolveNumberFromContext(def.amount, ctx),
      min: def.min,
      max: def.max,
  ...
  private createSetComponentValueEffect(def: SetComponentValueEffectDefinition): Effect {
    const componentName = this.getComponentName(def.component);
    ...
    if (def.values) {
      ...
      return new CustomEffect((ctx: ActionContext) => {
        ...
        for (const [key, value] of Object.entries(def.values!)) {
          (targetComponent as Record<string, unknown>)[key] = this.resolveValueFromContext(
            value,
            ctx
          );
        }
  ...
  private createCreateEntityEffect(def: CreateEntityEffectDefinition): Effect {
    const locationComponent = this.getComponentName('Location');
    const zoneListComponent = this.getComponentName('DeckList');
        ...
        components.push({
          ...
          data: (ctx: ActionContext) => {
            ...
            for (const [key, value] of Object.entries(data)) {
              resolved[key] = this.resolveValueFromContext(value, ctx);
            }
            return resolved;
          },
        });
    ...
    return new CreateEntity({
      components,
      zone: def.zone ? (ctx: ActionContext) => this.resolveZoneFromContext(def.zone!, ctx) : undefined,
      locationComponent,
      zoneListComponent,
      storeAs: def.storeAs,
      eventType: def.eventType,
  ...
  private createDestroyEntityEffect(def: DestroyEntityEffectDefinition): Effect {
    const locationComponent = this.getComponentName('Location');
    const zoneListComponent = this.getComponentName('DeckList');
    ...
    return new DestroyEntity({
      ...
      discardZone: def.discardZone
        ? (ctx: ActionContext) => this.resolveZoneFromContext(def.discardZone!, ctx)
        : undefined,
      locationComponent,
      zoneListComponent,
      eventType: def.eventType,
  ...
  private createTransferMultipleEffect(def: TransferMultipleEffectDefinition): Effect {
    const locationComponent = this.getComponentName('Location');
    const zoneListComponent = this.getComponentName('DeckList');

    return new TransferMultiple({
      fromZone: (ctx: ActionContext) => this.resolveZoneFromContext(def.fromZone, ctx),
      toZone: (ctx: ActionContext) => this.resolveZoneFromContext(def.toZone, ctx),
      count: (ctx: ActionContext) => this.resolveNumberFromContext(def.count, ctx),
  ...
  private createConditionalEffect(def: ConditionalEffectDefinition): Effect {
    ...
    return new ConditionalEffect({
      condition: (ctx: ActionContext) => {
        ...
        return this.resolver.evaluateCondition(def.condition, resolverContext);
      },
      thenEffect:
        thenEffects.length === 1 ? thenEffects[0] : new CompositeEffect(thenEffects),
  ...
  private createRepeatEffect(def: RepeatEffectDefinition): Effect {
    ...
    return new RepeatEffect({
      effect,
      times: (ctx: ActionContext) => this.resolveNumberFromContext(def.times, ctx),
    });
  }
