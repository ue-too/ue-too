...
export class PreconditionFactory {
    ...
    createPrecondition(definition: ConditionDefinition): Precondition {
        switch (definition.type) {
            ...
            case 'not':
                return new NotPrecondition(
                    this.createPrecondition(definition.condition)
                );

            default:
                throw new Error(
                    `Unknown condition type: ${(definition as ConditionDefinition).type}`
                );
        }
    }
    ...
    private createResourceCheck(
        ...
        return new ResourceCheck({
            ...
            value: (ctx: ActionContext) =>
                this.resolveNumberFromContext(def.value, ctx),
    ...
    private createEntityInZone(
        ...
        return new EntityInZone({
            entity: (ctx: ActionContext) =>
                this.resolveEntityFromContext(def.entity, ctx),
            zone: zones.map(
                z => (ctx: ActionContext) => this.resolveZoneFromContext(z, ctx)
            ),
            locationComponent,
            errorMessage: def.errorMessage,
        });
        ...
        if (def.operator) {
            ...
            return new ComponentValueCheck({
                ...
                value: (currentValue: unknown, ctx: ActionContext) => {
                    const expectedValue = this.resolveValueFromContext(
                        def.value,
                        ctx
                    );
    ...
    private createOwnerCheck(
        ...
        return new OwnerCheck({
            entity: (ctx: ActionContext) =>
                this.resolveEntityFromContext(def.entity, ctx),
            expectedOwner: (ctx: ActionContext) =>
                this.resolveEntityFromContext(def.expectedOwner, ctx),
            ownerComponent,
            ownerProperty: 'owner',
            invert: def.invert,
            errorMessage: def.errorMessage,
