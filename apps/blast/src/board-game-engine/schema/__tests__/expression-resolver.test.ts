...
describe('ExpressionResolver', () => {
        ...
        mockContext = {
            state: {
                ...
                getZone: (zoneName, owner) => {
                    if (zoneName === 'hand' && owner === player1)
                        return handZone;
                    return null;
                },
            },
    ...
    describe('resolveNumber', () => {
        ...
        it('should resolve $component.$actor.Resource.mana', () => {
            const result = resolver.resolveNumber(
                '$component.$actor.Resource.mana',
                mockContext
            );
            expect(result).toBe(5);
        });
    ...
    describe('resolveZone', () => {
        it('should resolve $zone.actor.hand', () => {
            const result = resolver.resolveZone(
                '$zone.actor.hand',
                mockContext
            );
    ...
    describe('resolveValue', () => {
        it('should resolve string expressions', () => {
            const result = resolver.resolveValue<number>(
                '$param.damage',
                mockContext
            );
            expect(result).toBe(5);
