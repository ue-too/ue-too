 ...
 */
import type { GameState, Phase, PhaseDefinition } from '../core/types';
...
export class PhaseManager {
    ...
    getCurrentPhase(state: GameState): Phase {
        ...
        if (!definition) {
            throw new Error(
                `Phase definition not found for phase: ${state.currentPhase}`
            );
        }
    ...
    advancePhase(state: GameState): GameState {
        ...
        if (!currentDef) {
            throw new Error(
                `Current phase definition not found: ${currentPhaseName}`
            );
        }
        ...
        if (!nextDef) {
            throw new Error(
                `Next phase definition not found: ${nextPhaseName}`
            );
        }
    ...
    private getNextPhase(
        ...
        if (!currentDef.nextPhase) {
            throw new Error(
                `No next phase defined for phase: ${currentDef.name}`
            );
        }
