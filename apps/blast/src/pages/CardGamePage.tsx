...
function getPlayerZone(engine: GameEngine, playerId: Entity, zoneName: string): Entity | null {
  const allEntities = engine.state.coordinator.getAllEntities();
  for (const entity of allEntities) {
    const zoneComp = engine.state.coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, entity);
    if (zoneComp && zoneComp.name === zoneName && zoneComp.owner === playerId) {
      return entity;
    }
  }
  return null;
...
function getCardsInZone(engine: GameEngine, zoneEntity: Entity): CardInfo[] {
  const deckComp = engine.state.coordinator.getComponentFromEntity<{ cached: { entities: Entity[] } }>(
    DECK_COMPONENT,
    zoneEntity
  );
  if (!deckComp) return [];
  ...
  for (const cardId of deckComp.cached.entities) {
    const cardComp = engine.state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, cardId);
    if (!cardComp) continue;

    const cardState = engine.state.coordinator.getComponentFromEntity<CardStateComponent>(
      CARD_STATE_COMPONENT,
      cardId
    );
    const zoneComp = engine.state.coordinator.getComponentFromEntity<ZoneComponent>(ZONE_COMPONENT, zoneEntity);
...
function CardDisplay({ card, isSelected, onClick, canSelect, showCost = true }: CardDisplayProps) {
  ...
  return (
    <div style={cardStyle} onClick={canSelect ? onClick : undefined}>
      <div style={{ fontWeight: 'bold', fontSize: '12px', marginBottom: '5px' }}>{cardComp.name}</div>
      {showCost && (
        <div style={{ fontSize: '10px', color: '#666' }}>
          Cost: <span style={{ color: '#2196F3', fontWeight: 'bold' }}>{cardComp.cost}</span>
        </div>
      )}
      {cardComp.cardType === 'Creature' && (
        <div style={{ fontSize: '11px', marginTop: '5px' }}>
          ...
          {' / '}
          <span style={{ color: '#4CAF50' }}>🛡 {cardComp.toughness}</span>
        </div>
      )}
      {cardState?.tapped && <div style={{ fontSize: '10px', color: '#999' }}>Tapped</div>}
      {cardState?.summoningSickness && <div style={{ fontSize: '10px', color: '#ff9800' }}>Sick</div>}
    </div>
  );
}
...
export function CardGamePage() {
  ...
  const [actionMode, setActionMode] = useState<'normal' | 'attackCreature' | 'attackPlayer'>('normal');
  ...
  const executeAction = (type: string, targetIds: Entity[] = []) => {
    ...
    const action = actions.find(
      (a) => a.type === type && JSON.stringify(a.targetIds) === JSON.stringify(targetIds)
    );
    if (action) {
    ...
    if (actionMode === 'attackCreature') {
      if (owner !== currentPlayer.playerId && zone === 'board') {
        ...
        if (selectedCard !== null) {
          executeAction('AttackCreature', [selectedCard, card.cardId]);
        }
      }
      ...
      if (owner === currentPlayer.playerId) {
        if (zone === 'hand') {
          ...
          setSelectedCard(selectedCard === card.cardId ? null : card.cardId);
        } else if (zone === 'board') {
  ...
  const validActions = useMemo(() => engine.getValidActions(), [engine, renderCount]);
  const canDraw = validActions.some((a) => a.type === 'DrawCard');
  const canPlaySelected =
    selectedCard !== null && validActions.some((a) => a.type === 'PlayCard' && a.targetIds[0] === selectedCard);
  const canAttackWithSelected =
    selectedCard !== null &&
    validActions.some((a) => (a.type === 'AttackCreature' || a.type === 'AttackPlayer') && a.targetIds[0] === selectedCard);
    ...
    <div style={{ padding: '20px', fontFamily: 'system-ui, sans-serif', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f0f0f0', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          ...
          {currentPlayer && (
            <span>
              <strong>Active:</strong> {currentPlayer.playerComp?.name}
            </span>
          )}
          {isGameOver && (
      ...
      {actionMode !== 'normal' && (
        ...
        >
          <span>
            {actionMode === 'attackCreature' && 'Select an enemy creature to attack'}
            {actionMode === 'attackPlayer' && 'Click "Attack Player" button next to opponent'}
          </span>
          <button onClick={handleCancelAction} style={{ padding: '5px 10px' }}>
            Cancel
          </button>
        </div>
      )}
      ...
      <div style={{ marginTop: '20px' }}>
        {allPlayers.map(({ playerId, playerComp, resourceComp }) => {
          const isCurrentPlayer = playerId === currentPlayer?.playerId;
          ...
          const handCards = handZone ? getCardsInZone(engine, handZone) : [];
          const boardCards = boardZone ? getCardsInZone(engine, boardZone) : [];
          const deckSize = deckZone
            ? (engine.state.coordinator.getComponentFromEntity<{ cached: { entities: Entity[] } }>(
                DECK_COMPONENT,
                deckZone
              )?.cached.entities.length ?? 0)
            : 0;
          ...
          return (
            <div
              ...
              style={{
                padding: '15px',
                margin: '10px 0',
                border: isCurrentPlayer ? '3px solid #4CAF50' : '1px solid #ddd',
                borderRadius: '8px',
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ margin: 0 }}>
                  {playerComp?.name}
                  {isCurrentPlayer && <span style={{ color: '#4CAF50', marginLeft: '10px' }}>● Active</span>}
                </h3>
                {!isCurrentPlayer && isCurrentPlayer === false && canAttackWithSelected && (
                  <button
                    onClick={() => handleAttackPlayer(playerId)}
                    style={{
                      padding: '5px 10px',
              ...
              <div style={{ display: 'flex', gap: '20px', marginTop: '10px' }}>
                <div>
                  ...
                  <span style={{ color: resourceComp && resourceComp.health <= 5 ? '#f44336' : '#4CAF50' }}>
                    {resourceComp?.health}/{resourceComp?.maxHealth}
                  </span>
                </div>
                <div>
                  <strong>Mana:</strong>{' '}
                  <span style={{ color: '#2196F3' }}>
                    {resourceComp?.mana}/{resourceComp?.maxMana}
                  </span>
                </div>
                <div>
              ...
              <div style={{ marginTop: '15px' }}>
                <strong>Board ({boardCards.length} creatures):</strong>
                <div style={{ minHeight: '80px', padding: '10px', backgroundColor: '#e8f5e9', borderRadius: '4px' }}>
                  {boardCards.length === 0 ? (
                    <span style={{ color: '#999' }}>No creatures on board</span>
                  ) : (
                    boardCards.map((card) => (
                      <CardDisplay
                        key={card.cardId}
                        ...
                        onClick={() => handleCardClick(card, 'board', playerId)}
                        canSelect={isCurrentPlayer || actionMode === 'attackCreature'}
                        showCost={false}
                      />
                    ))
                  )}
              ...
              {isCurrentPlayer && (
                <div style={{ marginTop: '15px' }}>
                  <strong>Hand ({handCards.length} cards):</strong>
                  <div style={{ minHeight: '80px', padding: '10px', backgroundColor: '#e3f2fd', borderRadius: '4px' }}>
                    {handCards.length === 0 ? (
                      <span style={{ color: '#999' }}>No cards in hand</span>
                    ) : (
                      handCards.map((card) => (
                        <CardDisplay
                          key={card.cardId}
                          card={card}
                          isSelected={selectedCard === card.cardId}
                          onClick={() => handleCardClick(card, 'hand', playerId)}
                          canSelect={true}
                          showCost={true}
                        />
                      ))
      ...
      <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              padding: '10px 20px',
              backgroundColor: canDraw && !isGameOver ? '#2196F3' : '#ccc',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: canDraw && !isGameOver ? 'pointer' : 'not-allowed',
            }}
          >
            Draw Card
          </button>
          ...
          <button
            ...
            style={{
              padding: '10px 20px',
              backgroundColor: canPlaySelected && !isGameOver ? '#4CAF50' : '#ccc',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: canPlaySelected && !isGameOver ? 'pointer' : 'not-allowed',
            }}
          >
            Play Selected
          </button>
          ...
          <button
            ...
            style={{
              padding: '10px 20px',
              backgroundColor: canAttackWithSelected && !isGameOver ? '#f44336' : '#ccc',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: canAttackWithSelected && !isGameOver ? 'pointer' : 'not-allowed',
            }}
          >
            Attack Creature
          </button>
          ...
          <button
            ...
            style={{
              padding: '10px 20px',
              backgroundColor: canEndTurn && !isGameOver ? '#9c27b0' : '#ccc',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: canEndTurn && !isGameOver ? 'pointer' : 'not-allowed',
            }}
          >
            End Turn
          </button>
      ...
      <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
        ...
        <details style={{ marginTop: '10px' }}>
          <summary style={{ cursor: 'pointer' }}>Available Actions</summary>
          <ul style={{ fontSize: '12px', maxHeight: '200px', overflow: 'auto' }}>
            {validActions.map((action, idx) => (
              <li key={idx}>
                {action.type}
                {action.targetIds.length > 0 && ` (targets: ${action.targetIds.join(', ')})`}
              </li>
            ))}
          </ul>
        </details>
