...
import type { GameDefinition, PhaseDefinition, Rule, ActionContext } from '../../board-game-engine';

// Core components (reuse these)
import { PLAYER_COMPONENT, ZONE_COMPONENT } from '../../board-game-engine/core/game-state';
```
...
```typescript
import { createGlobalComponentName, ComponentName, Coordinator, Entity } from '@ue-too/ecs';
...
export const LOCATION_COMPONENT: ComponentName = createGlobalComponentName('Location');
...
import { CustomEffect, EmitEvent } from '../../board-game-engine/action-system/effects';
import { IsPlayerTurn, PhaseCheck, CustomPrecondition } from '../../board-game-engine/action-system/preconditions';
import {
  CARD_COMPONENT,
  RESOURCE_COMPONENT,
  LOCATION_COMPONENT,
...
function getCardsInZone(ctx: { state: any }, zoneName: string, owner: Entity): Entity[] {
  ...
  for (const entity of coordinator.getAllEntities()) {
    ...
    const locationComp = coordinator.getComponentFromEntity(LOCATION_COMPONENT, entity);
    if (!locationComp) continue;

    const zoneComp = coordinator.getComponentFromEntity(ZONE_COMPONENT, locationComp.location);
...
export const playCardAction = new ActionDefinition({
  ...
  preconditions: [
    ...
    new CustomPrecondition(
      (ctx) => {
        ...
        const locationComp = ctx.state.coordinator.getComponentFromEntity(LOCATION_COMPONENT, card);
        if (!locationComp) return false;

        const zoneComp = ctx.state.coordinator.getComponentFromEntity(ZONE_COMPONENT, locationComp.location);
    ...
    new CustomPrecondition(
      (ctx) => {
        const card = ctx.targets[0];
        const cardComp = ctx.state.coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, card);
        const resources = ctx.state.coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, ctx.actor);
  ...
  effects: [
    ...
    new CustomEffect((ctx) => {
      ...
      for (const entity of coordinator.getAllEntities()) {
        const zoneComp = coordinator.getComponentFromEntity(ZONE_COMPONENT, entity);
  ...
  targetSelector: (state, actor) => {
    ...
    const resources = state.coordinator.getComponentFromEntity<ResourceComponent>(RESOURCE_COMPONENT, actor);
...
function createInitialState(): GameState {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    coordinator.addComponentToEntity<PlayerComponent>(PLAYER_COMPONENT, player, {
      name: `Player ${i + 1}`,
      playerNumber: i,
    });

    coordinator.addComponentToEntity<ResourceComponent>(RESOURCE_COMPONENT, player, {
      mana: 1,
      maxMana: 10,
      health: 20,
      maxHealth: 20,
...
new CustomEffect((ctx) => {
  ...
  // Update location
  const location = coordinator.getComponentFromEntity<LocationComponent>(LOCATION_COMPONENT, card);
...
```typescript
// game-definition.ts
import { GameEngine, type GameDefinition } from '../../board-game-engine';
...
```typescript
function getCardsInZone(coordinator: Coordinator, zoneName: string, owner: Entity): Entity[] {
  // ...
}
...
```typescript
const cardComp = coordinator.getComponentFromEntity<CardComponent>(CARD_COMPONENT, entity);
