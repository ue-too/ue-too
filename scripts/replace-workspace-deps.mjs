...
function getPackageVersion(packagePath) {
    try {
        ...
        console.warn(
            `Could not read version from ${packagePath}:`,
            error.message
        );
        return null;
    }
                ...
                if (version === 'workspace:*') {
                    ...
                    if (depPackagePath) {
                        ...
                        if (depVersion) {
                            ...
                            modified = true;
                            console.log(
                                `Updated ${dep} from workspace:* to ^${depVersion} in ${packagePath}`
                            );
                        }
                    }
                }
...
// Main execution
console.log(
    'Replacing workspace dependencies with actual versions in dist folders...'
);
const distPackageFiles = getDistPackageJsonFiles();

if (distPackageFiles.length === 0) {
    console.log(
        'No dist folders found. Please build packages first using: pnpm build'
    );
    process.exit(1);
}
